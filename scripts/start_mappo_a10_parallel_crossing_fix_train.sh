...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  echo "usage: start_mappo_a10_parallel_crossing_fix_train.sh [run_name] [extra train args...]"
  echo "env: NUM_SAMPLER_WORKERS(default=2) BASE_ROS_DOMAIN_ID(default=180)"
  exit 0
fi

RUN_NAME="${1:-mappo_dense_a10_parallel_crossing_fix_$(date +%Y%m%d_%H%M%S)}"
...
exec ./scripts/start_mappo_a10_parallel_train.sh \
  ...
  --head-on-target-starboard-offset 1.35 \
  --head-on-corridor-reward-weight 4.1 \
  --head-on-centerline-penalty-weight 4.6 \
  --head-on-turn-reward-weight 1.9 \
  --head-on-forward-reward-weight 0.32 \
  --head-on-speed-drop-penalty-weight 0.55 \
