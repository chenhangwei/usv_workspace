#!/bin/bash
# fresh103 crossing-only triage for near-goal finish candidates.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh103_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh103_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-380}
...
FRESH103_LATEST_COUNT=${FRESH103_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh103_near_goal_finish.pt

mapfile -t latest_fresh103 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh103_checkpoints/fresh103_near_goal_finish_step_*.pt 2>/dev/null | head -n "$FRESH103_LATEST_COUNT")
for model in "${latest_fresh103[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh103 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh103_focus_${label}.log"
...
echo "[$(date +%T)] fresh103 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"
