#!/bin/bash
# fresh96 pipeline: train, selected-evaluate, then print crossing-specific ranking.
...
LOG="${LOG:-/tmp/fresh96_pipeline.log}"
export PYTHONUNBUFFERED=1

{
  echo "[$(date +%T)] ==== fresh96 pipeline start ===="
  echo "[$(date +%T)] Train log: ${LOG_FILE:-/tmp/fresh96_train.log}"
  bash src/scripts/train_fresh96.sh
  echo "[$(date +%T)] ==== training complete; starting selected eval ===="
  bash src/scripts/fresh96_selected_eval.sh
  echo "[$(date +%T)] ==== selected eval complete; parsing crossing summaries ===="
  /bin/python3 - <<'PY'
import json
from pathlib import Path

eval_dir = Path('/mnt/data/checkpoints/usv_rl/fresh96_selected_eval')
...
PY
  echo "[$(date +%T)] ==== fresh96 pipeline complete ===="
} 2>&1 | tee "$LOG"
