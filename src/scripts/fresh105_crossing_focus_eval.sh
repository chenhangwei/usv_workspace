#!/bin/bash
# fresh105 crossing-only triage for wide-finish-band candidates.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh105_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh105_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-380}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-420}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-125}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-68}
BASE_DOMAIN=${BASE_DOMAIN:-181}
FRESH105_LATEST_COUNT=${FRESH105_LATEST_COUNT:-12}
...
if [[ "$INCLUDE_BASELINE" == "1" ]]; then
  ...
  add_candidate /mnt/data/checkpoints/usv_rl/fresh104_checkpoints/fresh104_goal_hold_step_0004228.pt
  add_candidate /mnt/data/checkpoints/usv_rl/fresh103_checkpoints/fresh103_near_goal_finish_step_0002880.pt
fi
add_candidate /mnt/data/checkpoints/usv_rl/fresh105_wide_finish_band.pt

mapfile -t latest_fresh105 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh105_checkpoints/fresh105_wide_finish_band_step_*.pt 2>/dev/null | head -n "$FRESH105_LATEST_COUNT")
for model in "${latest_fresh105[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh105 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh105_focus_${label}.log"
  domain=$((BASE_DOMAIN + (idx % 36)))
  idx=$((idx + 1))
...
echo "[$(date +%T)] fresh105 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"