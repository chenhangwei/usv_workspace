#!/bin/bash
# fresh131 pipeline: train activation-unblocked separation polish and run crossing triage.
...
LOG="${LOG:-/tmp/fresh131_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh131_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh131_active_sep_floor_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh131_checkpoints}"
...
{
  echo "========== fresh131 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh131.sh
  echo "========== fresh131 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh131_active_sep_floor_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh131_crossing_summary.tsv"
...
print(
    f"fresh131 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh131 crossing summary =========="
  cat "$summary"
  echo "========== fresh131 pipeline complete =========="
