...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh110_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh110_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
...
FRESH110_LATEST_COUNT=${FRESH110_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh110_stronger_close_finish.pt

mapfile -t latest_fresh110 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh110_checkpoints/fresh110_stronger_close_finish_step_*.pt 2>/dev/null | head -n "$FRESH110_LATEST_COUNT")
for model in "${latest_fresh110[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh110 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh110_focus_${label}.log"
...
echo "[$(date +%T)] fresh110 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"