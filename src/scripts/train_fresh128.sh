#!/bin/bash
# fresh128: balanced finish/safety continuation from fresh124 step0576.
#
# fresh127 improved some separation/omega metrics but made lagging boats too
# conservative.  This run keeps the fresh124 finish dynamics and adds only a
# narrow trainer-side safety brake for close post-conflict samples.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh128_balanced_finish_from_fresh124_step0576.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh128_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh128_train.log}"
export RUN_NAME="${RUN_NAME:-fresh128}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --lagging-finish-target-speed 0.19 \
  ...
  --lagging-finish-hold-weight 1.4 \
  --team-safety-brake-weight 0.45 \
  --team-safety-brake-weight-end 0.12 \
  ...
  --team-safety-brake-omega-weight 0.03 \
  --team-safety-brake-target-omega 0.04 \
  --team-safety-brake-turn-mode away \
  --team-safety-brake-require-neighbor \
  --team-safety-brake-power 1.60 \
