#!/bin/bash
# Fixed-seed random-only screening for fresh171.
...
OUT="${OUT:-/tmp/fresh171_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh171_inside_goal_hold_from_fresh168.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh171_checkpoints/fresh171_inside_goal_hold_from_fresh168_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh171 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh171-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
