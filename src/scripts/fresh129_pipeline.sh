#!/bin/bash
# fresh129 pipeline: train completion-regain continuation and crossing triage.
...
LOG="${LOG:-/tmp/fresh129_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh129_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh129_finish_regain_from_fresh128.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh129_checkpoints}"
...
{
  echo "========== fresh129 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh129.sh
  echo "========== fresh129 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh129_finish_regain_from_fresh128_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh129_crossing_summary.tsv"
...
print(
    f"fresh129 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh129 crossing summary =========="
  cat "$summary"
  echo "========== fresh129 pipeline complete =========="
