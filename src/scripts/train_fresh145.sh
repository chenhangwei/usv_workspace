#!/bin/bash
# fresh145: low-CTE low-omega disentanglement refinement from fresh142.
#
# User target:
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh145_low_cte_omega_disentangle_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh145_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh145_train.log}"
RUN_NAME="${RUN_NAME:-fresh145}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-864}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-196}"
...
{
  echo "========== ${RUN_NAME} Low-CTE Low-Omega Disentanglement (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --near-miss-distance 1.10 \
  --near-miss-weight 230.0 \
  --near-miss-exponent 2.55 \
  --head-on-near-miss-distance 1.22 \
  ...
  --team-regression-penalty-weight 2.4 \
  --team-dispersion-penalty-weight 0.06 \
  --team-dispersion-margin 0.55 \
  --deadlock-penalty-weight 12.0 \
  --entanglement-penalty-weight 15.0 \
  --entanglement-distance 3.4 \
  --entanglement-grace-steps 2 \
  --entanglement-low-speed-penalty-weight 2.80 \
  --path-deviation-penalty-weight 5.2 \
  --path-deviation-tolerance 0.45 \
  --path-deviation-conflict-scale 0.86 \
  --cte-clip-range 4.0 \
  --heading-error-weight 1.70 \
  --heading-relief-factor 0.62 \
  --heading-convergence-reward-weight 0.88 \
  --heading-convergence-threshold-deg 7.0 \
  --heading-correction-reward-weight 0.95 \
  --action-smoothness-weight 6.8 \
  --angular-accel-penalty-weight 4.2 \
  --straight-line-omega-penalty-weight 24.0 \
  --straight-line-omega-conflict-floor 0.035 \
  --straight-line-omega-cte-gate 0.42 \
  --saturated-omega-flip-penalty-weight 34.0 \
  --forward-speed-change-penalty-weight 0.95 \
  --omega-flip-saturation-threshold 0.12 \
  --pure-cruise-reward-weight 0.16 \
  --pure-idle-penalty-weight 0.10 \
  --pure-turn-penalty-weight 1.05 \
  --pure-spin-penalty-weight 1.25 \
...
echo "${RUN_NAME} low-CTE low-omega disentanglement complete: $OUTPUT" | tee -a "$LOG_FILE"
