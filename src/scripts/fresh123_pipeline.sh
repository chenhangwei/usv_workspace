#!/bin/bash
# fresh123 pipeline: train starboard commitment and run crossing triage.
...
LOG="${LOG:-/tmp/fresh123_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh123_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh123_starboard_commit.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh123_checkpoints}"
FRESH109="${FRESH109:-/mnt/data/checkpoints/usv_rl/fresh109_near_team_lagging_finish.pt}"
FRESH122="${FRESH122:-/mnt/data/checkpoints/usv_rl/fresh122_checkpoints/fresh122_early_brake_escape_turn_step_0000576.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh123 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh123.sh
  echo "========== fresh123 train complete; crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH109" ]] && candidates+=("$FRESH109")
  [[ -f "$FRESH122" ]] && candidates+=("$FRESH122")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh123_starboard_commit_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh123_crossing_summary.tsv"
...
print(
    f"fresh123 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh123 crossing summary =========="
  cat "$summary"
  echo "========== fresh123 pipeline complete =========="
