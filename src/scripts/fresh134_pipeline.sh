#!/bin/bash
# fresh134 pipeline: finish-first local-guard training and paired crossing triage.
...
LOG="${LOG:-/tmp/fresh134_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh134_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
EVAL_TIMEOUT="${EVAL_TIMEOUT:-520}"
EPISODE_TIMEOUT="${EPISODE_TIMEOUT:-135}"
NO_PROGRESS_TIMEOUT="${NO_PROGRESS_TIMEOUT:-105}"
BASE_DOMAIN="${BASE_DOMAIN:-216}"
MAX_DOMAIN="${MAX_DOMAIN:-232}"
SEED="${SEED:-1340}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh134_finish_first_local_guard_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh134_checkpoints}"
...
{
  echo "========== fresh134 pipeline =========="
  echo "Started: $(date -Iseconds)"
  echo "Eval seed: $SEED"
  bash src/scripts/train_fresh134.sh
  echo "========== fresh134 train complete; paired crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh134_finish_first_local_guard_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh134_crossing_summary.tsv"
...
print(
    f"fresh134 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh134 crossing summary =========="
  cat "$summary"
  echo "========== fresh134 pipeline complete =========="
