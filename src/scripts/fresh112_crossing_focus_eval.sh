...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh112_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh112_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-450}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-135}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-105}
BASE_DOMAIN=${BASE_DOMAIN:-126}
FRESH112_LATEST_COUNT=${FRESH112_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh112_raw_target_last5m.pt

mapfile -t latest_fresh112 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh112_checkpoints/fresh112_raw_target_last5m_step_*.pt 2>/dev/null | head -n "$FRESH112_LATEST_COUNT")
for model in "${latest_fresh112[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh112 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh112_focus_${label}.log"
...
echo "[$(date +%T)] fresh112 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"