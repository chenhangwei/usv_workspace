#!/bin/bash
# fresh120 pipeline: train team safety brake, then crossing triage.
...
LOG="${LOG:-/tmp/fresh120_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh120_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh120_post_goal_team_brake.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh120_checkpoints}"
...
{
  echo "========== fresh120 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh120.sh
  echo "========== fresh120 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh120_post_goal_team_brake_step_*.pt 2>/dev/null || true)
  fi

  declare -A seen=()
  idx=0
  summary="$OUT/fresh120_crossing_summary.tsv"
...
print(
    f"fresh120 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh120 crossing summary =========="
  cat "$summary"
  echo "========== fresh120 pipeline complete =========="
