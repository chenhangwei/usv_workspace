#!/bin/bash
# fresh100 crossing-only triage. Expensive selected eval runs only after a
# crossing collision-rate breakthrough.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh100_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh100_crossing_focus_eval.log}
EPISODES=${EPISODES:-5}
STEPS=${STEPS:-360}
...
FRESH100_LATEST_COUNT=${FRESH100_LATEST_COUNT:-18}
...
mapfile -t latest_fresh100 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh100_checkpoints/fresh100_crossing_pretrain_role_step_*.pt 2>/dev/null | head -n "$FRESH100_LATEST_COUNT")
...
if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh100 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh100_focus_${label}.log"
...
echo "[$(date +%T)] fresh100 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"