#!/bin/bash
# Fixed-seed random-only screening for fresh175.
...
OUT="${OUT:-/tmp/fresh175_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh175_local_safe_finish_from_fresh172_step1260.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh175_checkpoints/fresh175_local_safe_finish_from_fresh172_step1260_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh175 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh175-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
