#!/bin/bash
# fresh170: split random goal-hold from after-clear low-priority finish.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh170_split_hold_release_from_fresh168.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh170_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh170_train.log}"
RUN_NAME="${RUN_NAME:-fresh170}"
...
{
  echo "========== ${RUN_NAME} Split Hold Release (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-weight 5.05 \
  --random-deconflict-weight-end 4.70 \
  ...
  --random-deconflict-release-separation 2.72 \
  ...
  --random-safe-finish-weight 1.85 \
  --random-safe-finish-weight-end 2.15 \
  --random-safe-finish-goal-tolerance 0.8 \
  --random-safe-finish-max-distance 13.0 \
  --random-safe-finish-phase-min -1.0 \
  --random-safe-finish-min-team-separation 2.65 \
  ...
  --random-safe-finish-low-priority-speed-multiplier 1.75 \
  --random-safe-finish-max-omega 0.065 \
  --random-safe-finish-omega-weight 0.09 \
  --random-goal-hold-weight 3.20 \
  --random-goal-hold-weight-end 4.20 \
  --random-goal-hold-distance 1.15 \
  --random-goal-hold-phase-min -1.0 \
  --random-goal-hold-target-speed 0.0 \
  --random-goal-hold-target-omega 0.0 \
  --random-goal-hold-omega-weight 1.20 \
  --policy-anchor-weight 225.0 \
  --policy-anchor-weight-end 265.0 \
...
echo "${RUN_NAME} split hold release fit complete: $OUTPUT" | tee -a "$LOG_FILE"
