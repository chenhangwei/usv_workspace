#!/bin/bash
# fresh144: balanced close-margin/finish refinement from fresh142.
#
# User target:
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh144_balanced_close_margin_finish_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh144_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh144_train.log}"
RUN_NAME="${RUN_NAME:-fresh144}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-864}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-192}"
...
{
  echo "========== ${RUN_NAME} Balanced Close-Margin Finish (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --separation-recovery-weight 1.8 \
  ...
  --path-deviation-penalty-weight 3.8 \
  --path-deviation-tolerance 0.54 \
  --path-deviation-conflict-scale 0.74 \
  --cte-clip-range 4.0 \
  ...
  --angular-accel-penalty-weight 2.9 \
  --straight-line-omega-penalty-weight 15.0 \
  --straight-line-omega-conflict-floor 0.05 \
  --straight-line-omega-cte-gate 0.52 \
  --saturated-omega-flip-penalty-weight 19.0 \
  ...
  --pure-turn-penalty-weight 0.66 \
  --pure-spin-penalty-weight 0.92 \
  ...
  --near-goal-finish-weight 2.55 \
  --near-goal-finish-weight-end 1.15 \
  ...
  --near-goal-finish-max-omega 0.052 \
  --near-goal-finish-omega-weight 0.26 \
  --lagging-finish-weight 1.92 \
  --lagging-finish-weight-end 1.00 \
  ...
  --lagging-finish-max-omega 0.038 \
  --lagging-finish-omega-weight 0.13 \
  ...
  --team-safety-brake-release-separation 1.08 \
  ...
  --policy-anchor-weight 1450.0 \
  --policy-anchor-weight-end 1750.0 \
...
echo "${RUN_NAME} balanced close-margin finish complete: $OUTPUT" | tee -a "$LOG_FILE"
