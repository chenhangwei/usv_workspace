#!/bin/bash
# fresh616: fresh615 (route-first + multi-waypoint) PLUS an explicit TURN-SPEED
# COUPLING reward, per the user's seamanship principle (2026-06-28): obtuse turn ->
# keep near full speed; acute turn -> slow down to carve a tight radius (R=v/omega).
#
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh616_turn_speed_coupling_3usv.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh616_checkpoints}"
...
echo "[fresh616] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh616] OUTPUT=$OUTPUT"
echo "[fresh616] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh616] fresh615 + TURN-SPEED COUPLING (acute->slow, obtuse->fast): weight 0.6 floor 0.5 deadband 25deg"
