#!/bin/bash
# fresh121 pipeline: train early team safety brake, then crossing triage.
...
LOG="${LOG:-/tmp/fresh121_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh121_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh121_early_team_brake.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh121_checkpoints}"
FRESH109="${FRESH109:-/mnt/data/checkpoints/usv_rl/fresh109_near_team_lagging_finish.pt}"
FRESH120_0576="${FRESH120_0576:-/mnt/data/checkpoints/usv_rl/fresh120_checkpoints/fresh120_post_goal_team_brake_step_0000576.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh121 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh121.sh
  echo "========== fresh121 train complete; crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH109" ]] && candidates+=("$FRESH109")
  [[ -f "$FRESH120_0576" ]] && candidates+=("$FRESH120_0576")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh121_early_team_brake_step_*.pt 2>/dev/null || true)
  fi

  declare -A seen=()
  idx=0
  summary="$OUT/fresh121_crossing_summary.tsv"
...
print(
    f"fresh121 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh121 crossing summary =========="
  cat "$summary"
  echo "========== fresh121 pipeline complete =========="
