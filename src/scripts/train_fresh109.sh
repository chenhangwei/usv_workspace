#!/bin/bash
# fresh109: low-noise rollout plus trainer-side near-complete lagging finish gate.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh109_near_team_lagging_finish.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh109_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh109_train.log}"
export RUN_NAME="${RUN_NAME:-fresh109}"
export TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-1152}"
export BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-211}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --actor-log-std-init -3.55 \
  --force-actor-log-std -3.55 \
