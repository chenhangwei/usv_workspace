#!/bin/bash
# fresh133 pipeline: train local-danger brake and run crossing triage.
...
LOG="${LOG:-/tmp/fresh133_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh133_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh133_local_danger_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh133_checkpoints}"
...
FRESH132_STEP0576="${FRESH132_STEP0576:-/mnt/data/checkpoints/usv_rl/fresh132_checkpoints/fresh132_neighbor_sep_floor_from_fresh124_step0576_step_0000576.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh133 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh133.sh
  echo "========== fresh133 train complete; crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH124" ]] && candidates+=("$FRESH124")
  [[ -f "$FRESH128" ]] && candidates+=("$FRESH128")
  [[ -f "$FRESH132_STEP0576" ]] && candidates+=("$FRESH132_STEP0576")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh133_local_danger_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh133_crossing_summary.tsv"
...
print(
    f"fresh133 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh133 crossing summary =========="
  cat "$summary"
  echo "========== fresh133 pipeline complete =========="
