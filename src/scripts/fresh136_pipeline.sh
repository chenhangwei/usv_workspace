#!/bin/bash
# fresh136 pipeline: full three-update fresh128 finish polish and seeded triage.
...
LOG="${LOG:-/tmp/fresh136_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh136_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
SEED="${SEED:-1360}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh136_active_finish_from_fresh128.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh136_checkpoints}"
...
{
  echo "========== fresh136 pipeline =========="
  echo "Started: $(date -Iseconds)"
  echo "Eval seed: $SEED"
  bash src/scripts/train_fresh136.sh
  echo "========== fresh136 train complete; paired crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh136_active_finish_from_fresh128_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh136_crossing_summary.tsv"
...
print(
    f"fresh136 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh136 crossing summary =========="
  cat "$summary"
  echo "========== fresh136 pipeline complete =========="
