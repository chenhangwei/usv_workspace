#!/bin/bash
# Fixed-seed random-only screening for fresh173.
...
OUT="${OUT:-/tmp/fresh173_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh173_local_offroute_from_fresh172_step1260.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh173_checkpoints/fresh173_local_offroute_from_fresh172_step1260_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh173 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh173-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
