#!/bin/bash
# fresh143: de-snake close-margin refinement from fresh142.
#
# User target:
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh143_desnake_close_margin_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh143_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh143_train.log}"
RUN_NAME="${RUN_NAME:-fresh143}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-864}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-188}"
...
{
  echo "========== ${RUN_NAME} De-Snake Close-Margin Refinement (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --near-miss-distance 1.02 \
  --near-miss-weight 210.0 \
  --near-miss-exponent 2.45 \
  --head-on-near-miss-distance 1.16 \
  ...
  --proximity-gradient-penalty-weight 7.5 \
  --proximity-gradient-distance 1.14 \
  --speed-distance-coupling-penalty-weight 13.0 \
  --speed-distance-coupling-threshold 1.12 \
  ...
  --entanglement-penalty-weight 11.0 \
  --entanglement-distance 3.0 \
  --entanglement-grace-steps 3 \
  --entanglement-low-speed-penalty-weight 1.90 \
  --path-deviation-penalty-weight 4.2 \
  --path-deviation-tolerance 0.52 \
  --path-deviation-conflict-scale 0.78 \
  --cte-clip-range 4.0 \
  ...
  --angular-accel-penalty-weight 3.8 \
  --straight-line-omega-penalty-weight 22.0 \
  --straight-line-omega-conflict-floor 0.04 \
  --straight-line-omega-cte-gate 0.48 \
  --saturated-omega-flip-penalty-weight 32.0 \
  ...
  --pure-turn-penalty-weight 0.86 \
  --pure-spin-penalty-weight 1.10 \
  ...
  --near-goal-finish-max-omega 0.045 \
  --near-goal-finish-omega-weight 0.30 \
  --lagging-finish-weight 1.80 \
  --lagging-finish-weight-end 0.95 \
  ...
  --team-safety-brake-release-separation 1.04 \
...
echo "${RUN_NAME} de-snake close-margin refinement complete: $OUTPUT" | tee -a "$LOG_FILE"
