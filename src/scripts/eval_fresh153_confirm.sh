#!/bin/bash
# Three-episode confirm for the fresh153 final checkpoint.
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh153_early_headon_brake_from_fresh150.pt}"
LABEL="${LABEL:-fresh153_early_headon_brake_from_fresh150}"
SUMMARY="${SUMMARY:-$OUT/fresh153_confirm_summary.tsv}"
EPISODES="${EPISODES:-3}"
STEPS="${STEPS:-420}"
...
print(
    f"fresh153-confirm {label} {scenario_name}: episodes={row['episodes']} "
...
printf '========== fresh153 confirm summary =========='"\n"
cat "$SUMMARY"