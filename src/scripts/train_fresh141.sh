#!/bin/bash
# fresh141: close-margin finish/generalization continuation from fresh140.
#
# User target:
#   - collision floor is 0.75 m; do not reward unnecessarily large avoidance gaps
#   - finish the last meters instead of timing out near goal
#   - when clear ahead, track the spawn-goal route straight with low CTE/S-curve
#   - strengthen random and dense multi-USV crossing/disengagement behavior
...
PREFERRED_BASE="/mnt/data/checkpoints/usv_rl/fresh140_guarded_finish_from_fresh139_step0864.pt"
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh141_close_margin_finish_generalize_from_fresh140.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh141_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh141_train.log}"
RUN_NAME="${RUN_NAME:-fresh141}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-960}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-176}"

mkdir -p "$CKPT_DIR"

if [[ -z "$BASE_INPUT" || ! -f "$BASE_INPUT" ]]; then
  echo "ERROR: base checkpoint not found. Set BASE_INPUT or finish fresh140 first."
  exit 1
fi
...
{
  echo "========== ${RUN_NAME} Close-Margin Finish Generalization (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --near-miss-weight 260.0 \
  --near-miss-exponent 2.65 \
  --head-on-near-miss-distance 1.35 \
  ...
  --proximity-gradient-distance 1.35 \
  --speed-distance-coupling-penalty-weight 16.0 \
  --speed-distance-coupling-threshold 1.30 \
  --separation-recovery-weight 5.2 \
  --avoidance-turn-reward-weight 3.4 \
  --team-reward-weight 0.68 \
  --team-progress-weight 1.34 \
  --team-goal-proximity-weight 0.36 \
  ...
  --path-deviation-penalty-weight 2.7 \
  --path-deviation-tolerance 0.68 \
  --path-deviation-conflict-scale 0.62 \
  --cte-clip-range 4.0 \
  --heading-error-weight 1.28 \
  --heading-relief-factor 0.72 \
  --heading-convergence-reward-weight 0.62 \
  --heading-convergence-threshold-deg 10.0 \
  --heading-correction-reward-weight 0.72 \
  --action-smoothness-weight 5.0 \
  --angular-accel-penalty-weight 2.4 \
  --straight-line-omega-penalty-weight 9.5 \
  --straight-line-omega-conflict-floor 0.06 \
  --straight-line-omega-cte-gate 0.55 \
  --saturated-omega-flip-penalty-weight 12.5 \
  --forward-speed-change-penalty-weight 0.76 \
  --omega-flip-saturation-threshold 0.18 \
  --pure-cruise-reward-weight 0.16 \
  --pure-idle-penalty-weight 0.10 \
  --pure-turn-penalty-weight 0.56 \
  --pure-spin-penalty-weight 0.80 \
  ...
  --goal-proximity-reward-weight 3.4 \
  ...
  --near-goal-finish-weight 2.40 \
  --near-goal-finish-weight-end 0.90 \
  ...
  --near-goal-finish-max-omega 0.060 \
  --near-goal-finish-omega-weight 0.22 \
  --lagging-finish-weight 1.65 \
  --lagging-finish-weight-end 0.86 \
  ...
  --team-safety-brake-release-separation 1.20 \
  ...
  --scenario-spawn-heading-std 0.035 \
...
echo "${RUN_NAME} close-margin finish generalization complete: $OUTPUT" | tee -a "$LOG_FILE"
