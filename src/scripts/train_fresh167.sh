#!/bin/bash
# fresh167: random safe-finish after deconflict danger clears.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh167_random_safe_finish_from_fresh163_step1152.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh167_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh167_train.log}"
RUN_NAME="${RUN_NAME:-fresh167}"
...
{
  echo "========== ${RUN_NAME} Random Safe Finish (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-weight 5.10 \
  --random-deconflict-weight-end 4.60 \
  ...
  --random-safe-finish-weight 1.35 \
  --random-safe-finish-weight-end 1.65 \
  --random-safe-finish-goal-tolerance 0.8 \
  --random-safe-finish-max-distance 13.0 \
  --random-safe-finish-phase-min -1.0 \
  --random-safe-finish-min-team-separation 2.55 \
  --random-safe-finish-full-team-separation 3.75 \
  ...
  --random-safe-finish-min-speed-scale 0.50 \
...
echo "${RUN_NAME} random safe-finish fit complete: $OUTPUT" | tee -a "$LOG_FILE"
