#!/bin/bash
# Three-episode confirm for a selected fresh154 checkpoint.
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh154_cpa_guard_finish_from_fresh153.pt}"
LABEL="${LABEL:-$(basename "$MODEL" .pt)}"
SUMMARY="${SUMMARY:-$OUT/fresh154_confirm_summary.tsv}"
EPISODES="${EPISODES:-3}"
STEPS="${STEPS:-420}"
...
print(
    f"fresh154-confirm {label} {scenario_name}: episodes={row['episodes']} "
...
printf '========== fresh154 confirm summary =========='"\n"
cat "$SUMMARY"