#!/bin/bash
# fresh147: soft clear-to-route gate from fresh142.
#
# Direction:
#   - do not continue fresh145's global strong CTE/omega punishment
#   - keep close-margin/finish behavior from fresh142/fresh144
#   - apply extra CTE/heading/omega discipline only when the forward cone and local neighborhood are clear
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh147_soft_clear_gate_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh147_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh147_train.log}"
RUN_NAME="${RUN_NAME:-fresh147}"
...
{
  echo "========== ${RUN_NAME} Soft Clear-To-Route Gate (${TOTAL_TIMESTEPS}) =========="
...
echo "${RUN_NAME} soft clear-to-route gate complete: $OUTPUT" | tee -a "$LOG_FILE"
