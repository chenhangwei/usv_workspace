...
EVAL_DIR=/mnt/data/checkpoints/usv_rl/fresh86_full5_eval
...
CANDIDATES=(
  ...
  /mnt/data/checkpoints/usv_rl/fresh85_overtaking_scenario_head.pt
)
POLICIES=(
  mappo
  mappo_router
...
for idx in "${!CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 240 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/fresh86_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh86_quickeval_${base}.log)" | tee -a "$LOG"
  else
