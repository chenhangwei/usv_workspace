#!/bin/bash
# Fixed-seed random-only screening for fresh170.
...
OUT="${OUT:-/tmp/fresh170_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh170_split_hold_release_from_fresh168.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh170_checkpoints/fresh170_split_hold_release_from_fresh168_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh170 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh170-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
