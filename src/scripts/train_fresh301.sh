#!/bin/bash
# fresh301: Stage-C two-vessel negotiation/deconfliction.
#
# Goal:
#   Build on fresh300 route discipline and train standard two-vessel encounters
#   before returning to dense three-vessel random encounters.

set -eo pipefail

cd "$(dirname "$0")/.."

BASE_INPUT="${BASE_INPUT:-/mnt/data/checkpoints/usv_rl/fresh300_clear_route_stageA_from_fresh254.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh301_two_vessel_stageC_from_fresh300.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh301_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh301_train.log}"
RUN_NAME="${RUN_NAME:-fresh301}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-3360}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-225}"

FRESH301_ARGS="
  --num-sampler-workers 1
  --checkpoint-interval 1120
  ...
  --head-on-forward-reward-weight 1.2
  ...
  --overtaking-starboard-turn-reward-weight 0.75
  --overtaking-forward-reward-weight 0.45
  --overtaking-corridor-reward-weight 1.10
  --overtaking-centerline-penalty-weight 1.20
  --overtaking-close-penalty-weight 1.00
  ...
  --random-clear-ahead-weight-end 5.20
  ...
  --random-deconflict-weight-end 5.30
  ...
  --random-deconflict-omega-weight 1.30
  --random-deconflict-standon-weight 0.18
  --random-deconflict-yield-weight 5.20
  --random-deconflict-pretrain-epochs 1
  --random-deconflict-pretrain-learning-rate 3.5e-7
  --random-deconflict-pretrain-max-grad-norm 0.030
  --random-role-balance-weight 4.60
  --random-role-balance-weight-end 5.10
  --random-role-balance-pretrain-epochs 1
  --random-role-balance-pretrain-learning-rate 2.8e-7
  ...
  --random-role-balance-yield-weight 2.35
  --random-role-balance-omega-weight 0.40
  --random-cte-recovery-weight 0.28
  --random-cte-recovery-weight-end 0.36
