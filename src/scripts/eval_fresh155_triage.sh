#!/bin/bash
# One-episode paired triage for fresh155 checkpoints.
...
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh155_eval}"
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh155_reset_curriculum_cpa_finish_from_fresh153.pt}"
SUMMARY="${SUMMARY:-$OUT/fresh155_triage_summary.tsv}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-420}"
...
SEED="${SEED:-1460}"
BASE_DOMAIN="${BASE_DOMAIN:-222}"

mkdir -p "$OUT"
printf 'label\tscenario\tcollision\tsuccess\ttimeout\tprogress\tgoal\tsep\tcte\tentanglement\tcolregs\tomega\tmodel\n' > "$SUMMARY"

candidates=(
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0000288.pt
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0000576.pt
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0000864.pt
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0001152.pt
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0001440.pt
  /mnt/data/checkpoints/usv_rl/fresh155_checkpoints/fresh155_reset_curriculum_cpa_finish_from_fresh153_step_0001728.pt
  "$FINAL_MODEL"
)
...
print(
    f"fresh155-triage {label} {scenario_name}: coll={row['collision']:.3f} "
    f"succ={row['success']:.3f} timeout={row['timeout']:.3f} progress={row['progress']:.3f} "
    f"goal={row['goal']:.3f} sep={row['sep']:.3f} cte={row['cte']:.3f} "
    f"ent={row['entanglement']:.3f} colregs={row['colregs']:.3f} omega={row['omega']:.1f}"
)
...
printf '========== fresh155 triage summary =========='"\n"
cat "$SUMMARY"