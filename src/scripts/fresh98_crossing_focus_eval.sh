#!/bin/bash
# fresh98 crossing-only triage. Prioritises three_usv_crossing before any aggregate evaluation.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh98_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh98_crossing_focus_eval.log}
EPISODES=${EPISODES:-5}
STEPS=${STEPS:-360}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-330}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-92}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-30}
BASE_DOMAIN=${BASE_DOMAIN:-173}
FRESH98_LATEST_COUNT=${FRESH98_LATEST_COUNT:-16}
FRESH97_LATEST_COUNT=${FRESH97_LATEST_COUNT:-2}
...
if (( FRESH97_LATEST_COUNT > 0 )); then
...
if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh98 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh98_focus_${label}.log"
...
echo "[$(date +%T)] fresh98 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"
