...
FINAL_CKPT="/mnt/data/checkpoints/usv_rl/fresh58.pt"
EVAL_DIR="/mnt/data/checkpoints/usv_rl/fresh58_eval"
RANKING="/mnt/data/checkpoints/usv_rl/fresh58_ranking.json"
EPISODES=5
STEPS=180

mkdir -p "$EVAL_DIR"

# Phase 2 checkpoints (skip phase1), 每隔 2 个取 1 个以节省时间
ALL_P2=$(ls "$CKPT_DIR"/fresh58_step_*.pt 2>/dev/null | sort -t_ -k3 -n)
CKPTS=""
IDX=0
for C in $ALL_P2; do
  if (( IDX % 2 == 0 )); then
...
echo "═══════════════════════════════════════════════════"
echo "  fresh58 Batch Evaluation"
echo "  Phase 2 checkpoints: $CKPT_COUNT (sampled every-2 + final)"
...
eval_dir = os.environ.get('EVAL_DIR', '/mnt/data/checkpoints/usv_rl/fresh58_eval')
ranking_path = os.environ.get('RANKING', '/mnt/data/checkpoints/usv_rl/fresh58_ranking.json')
