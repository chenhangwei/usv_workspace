#!/bin/bash
# fresh76 quick_eval — full 5-scenario eval for fresh76 head_on COLREGs-prior run
set -o pipefail
...
CANDIDATES=(
  /mnt/data/checkpoints/usv_rl/fresh75_best.pt
  /mnt/data/checkpoints/usv_rl/fresh76_headon_colregs.pt
)
for f in "$CKPT_DIR"/fresh76_headon_colregs_step_*.pt; do
  CANDIDATES+=("$f")
done

LOG=/tmp/fresh76_quickeval.log
: > "$LOG"
...
for model in "${CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 180 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/fresh76_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh76_quickeval_${base}.log)" | tee -a "$LOG"
  else
