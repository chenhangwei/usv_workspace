#!/bin/bash
# Fixed-seed random-only screening for fresh172.
...
OUT="${OUT:-/tmp/fresh172_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh172_offroute_recovery_from_fresh171.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh172_checkpoints/fresh172_offroute_recovery_from_fresh171_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh172 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh172-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
