#!/bin/bash
# fresh126 pipeline: train active wide post-conflict finish repair and crossing triage.
...
LOG="${LOG:-/tmp/fresh126_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh126_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh126_wide_finish_active_from_fresh101.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh126_checkpoints}"
...
{
  echo "========== fresh126 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh126.sh
  echo "========== fresh126 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh126_wide_finish_active_from_fresh101_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh126_crossing_summary.tsv"
...
print(
    f"fresh126 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh126 crossing summary =========="
  cat "$summary"
  echo "========== fresh126 pipeline complete =========="
