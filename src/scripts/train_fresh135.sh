#!/bin/bash
# fresh135: conservative completion polish from fresh128 final.
#
# fresh134 failed confirm after over-emphasizing raw-target finish from fresh124.
# This branch starts from the confirmed fresh128 final checkpoint, keeps the
# trusted finish/safety shape, and applies only a small completion polish with a
# weak local-danger guard.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh135_completion_polish_from_fresh128.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh135_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh135_train.log}"
export RUN_NAME="${RUN_NAME:-fresh135}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --lagging-finish-min-team-separation 1.22 \
  --lagging-finish-safe-team-separation 2.65 \
  --lagging-finish-safe-team-separation-power 1.10 \
  ...
  --team-safety-brake-weight-end 0.04 \
  --team-safety-brake-goal-tolerance 0.8 \
  --team-safety-brake-max-distance 6.2 \
  --team-safety-brake-phase-min 0.04 \
  ...
  --team-safety-brake-omega-weight 0.014 \
  --team-safety-brake-target-omega 0.030 \
  --team-safety-brake-turn-mode away \
  --team-safety-brake-require-neighbor \
  --team-safety-brake-local-danger \
  --team-safety-brake-power 1.50 \
