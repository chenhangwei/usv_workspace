...
EVAL_DIR=/mnt/data/checkpoints/usv_rl/fresh84_overtaking_scan
...
for checkpoint in /mnt/data/checkpoints/usv_rl/fresh84_checkpoints/fresh84_overtaking_scenario_residual_step_*.pt; do
  [[ -f "$checkpoint" ]] || continue
  CANDIDATES+=("$checkpoint")
  POLICIES+=(mappo)
done

CANDIDATES+=(/mnt/data/checkpoints/usv_rl/fresh84_overtaking_scenario_residual.pt)
POLICIES+=(mappo)

LOG=/tmp/fresh84_overtaking_scan.log
: > "$LOG"
...
for idx in "${!CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 180 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/${base}_fresh84_overtaking_eval.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/${base}_fresh84_overtaking_eval.log)" | tee -a "$LOG"
  else
