#!/bin/bash
# fresh149: anticipatory avoidance-turn shaping with soft clear-to-route gate from fresh142.
#
# Direction:
#   - do not continue fresh145's global strong CTE/omega punishment
#   - keep close-margin/finish behavior from fresh142/fresh144
#   - give closing neighbours a directional away-turn signal before near-miss range
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh149_anticipatory_turn_soft_clear_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh149_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh149_train.log}"
RUN_NAME="${RUN_NAME:-fresh149}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-480}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-208}"
...
{
  echo "========== ${RUN_NAME} Anticipatory Avoidance Turn (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --anticipatory-avoidance-turn-reward-weight 5.2 \
  --anticipatory-avoidance-turn-penalty-weight 8.5 \
...
echo "${RUN_NAME} anticipatory avoidance-turn shaping complete: $OUTPUT" | tee -a "$LOG_FILE"
