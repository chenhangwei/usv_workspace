#!/bin/bash
# fresh140 pipeline: guarded finish training plus one-episode crossing triage.
...
LOG="${LOG:-/tmp/fresh140_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh140_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
SEED="${SEED:-1390}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh140_guarded_finish_from_fresh139_step0864.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh140_checkpoints}"
...
{
  echo "========== fresh140 pipeline =========="
  echo "Started: $(date -Iseconds)"
  echo "Eval seed: $SEED"
  bash src/scripts/train_fresh140.sh
  echo "========== fresh140 train complete; paired crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH139_STEP0864" ]] && candidates+=("$FRESH139_STEP0864")
  [[ -f "$FRESH139_STEP0576" ]] && candidates+=("$FRESH139_STEP0576")
  [[ -f "$FRESH139_FINAL" ]] && candidates+=("$FRESH139_FINAL")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh140_guarded_finish_from_fresh139_step0864_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh140_crossing_summary.tsv"
...
print(
    f"fresh140 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh140 crossing summary =========="
  cat "$summary"
  echo "========== fresh140 pipeline complete =========="
