#!/bin/bash
# fresh60 检查点批量评估脚本
# 每隔2个checkpoint取样 + final，3场景 × 5 episodes
set -eo pipefail
...
FINAL_CKPT="/mnt/data/checkpoints/usv_rl/fresh60.pt"
EVAL_DIR="/mnt/data/checkpoints/usv_rl/fresh60_eval"
RANKING="/mnt/data/checkpoints/usv_rl/fresh60_ranking.json"
EPISODES=5
STEPS=180
...
# 每隔2个取样 + final
ALL_CKPTS=$(ls "$CKPT_DIR"/fresh60_step_*.pt 2>/dev/null | sort -t_ -k3 -n)
CKPTS=""
IDX=0
...
eval_dir = os.environ.get('EVAL_DIR', '/mnt/data/checkpoints/usv_rl/fresh60_eval')
ranking_path = os.environ.get('RANKING', '/mnt/data/checkpoints/usv_rl/fresh60_ranking.json')
...
echo "═══════════════════════════════════════════════════"
echo "  fresh60 Batch Evaluation Complete"
echo "═══════════════════════════════════════════════════"
