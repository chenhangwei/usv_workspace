...
if [[ -f "${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh514_arcturn.pt}" ]]; then
  RESUME_FROM="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh514_arcturn.pt}"
else
  RESUME_FROM="$(ls -1t /mnt/data/checkpoints/usv_rl/fresh514_checkpoints/*.pt 2>/dev/null | head -1 || true)"
fi
if [[ -z "${RESUME_FROM:-}" || ! -f "$RESUME_FROM" ]]; then
  echo "[resume514] ERROR: no checkpoint to resume from" >&2; exit 1
fi
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh514_arcturn.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh514_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-250}"

echo "[resume514] RESUME_FROM=$RESUME_FROM"
echo "[fresh514] OUTPUT=$OUTPUT"
echo "[fresh514] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh514] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
