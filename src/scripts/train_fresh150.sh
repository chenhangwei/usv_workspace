#!/bin/bash
# fresh150: CPA-margin effect shaping with soft clear-to-route gate from fresh142.
#
# Direction:
#   - do not keep raising scalar risk or pure turn reward
#   - keep fresh149's crossing guardrails and soft clear-to-route behavior
#   - reward measurable DCPA increase / closing-speed reduction before near miss
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh150_cpa_margin_soft_clear_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh150_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh150_train.log}"
RUN_NAME="${RUN_NAME:-fresh150}"
...
{
  echo "========== ${RUN_NAME} CPA Margin Effect Shaping (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --anticipatory-avoidance-turn-reward-weight 4.6 \
  --anticipatory-avoidance-turn-penalty-weight 7.2 \
  --anticipatory-avoidance-turn-distance 3.8 \
  --anticipatory-cpa-distance 4.8 \
  --anticipatory-cpa-time-horizon 9.0 \
  --anticipatory-dcpa-target 1.16 \
  --anticipatory-dcpa-deficit-penalty-weight 10.0 \
  --anticipatory-dcpa-improvement-reward-weight 7.4 \
  --anticipatory-closing-reduction-reward-weight 6.8 \
  --anticipatory-yield-speed 0.085 \
  --anticipatory-yield-speed-penalty-weight 16.0 \
  ...
  --policy-anchor-weight-end 2100.0 \
...
echo "${RUN_NAME} CPA margin effect shaping complete: $OUTPUT" | tee -a "$LOG_FILE"
