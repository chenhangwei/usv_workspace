#!/bin/bash
# fresh137 pipeline: conservative continuation and seeded crossing triage.
...
LOG="${LOG:-/tmp/fresh137_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh137_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
SEED="${SEED:-1370}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh137_stabilize_from_fresh136_step0864.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh137_checkpoints}"
...
{
  echo "========== fresh137 pipeline =========="
  echo "Started: $(date -Iseconds)"
  echo "Eval seed: $SEED"
  bash src/scripts/train_fresh137.sh
  echo "========== fresh137 train complete; paired crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh137_stabilize_from_fresh136_step0864_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh137_crossing_summary.tsv"
...
print(
    f"fresh137 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh137 crossing summary =========="
  cat "$summary"
  echo "========== fresh137 pipeline complete =========="
