#!/bin/bash
# fresh97 pipeline: train, selected-evaluate, then print crossing-specific ranking.
...
LOG="${LOG:-/tmp/fresh97_pipeline.log}"
export PYTHONUNBUFFERED=1

{
  echo "[$(date +%T)] ==== fresh97 pipeline start ===="
  echo "[$(date +%T)] Train log: ${LOG_FILE:-/tmp/fresh97_train.log}"
  bash src/scripts/train_fresh97.sh
    echo "[$(date +%T)] ==== training complete; running crossing-focus eval ===="
    bash src/scripts/fresh97_crossing_focus_eval.sh
    set +e
    /bin/python3 - <<'PY'
import json
from pathlib import Path

summary_path = Path('/mnt/data/checkpoints/usv_rl/fresh97_crossing_focus_eval/crossing_focus_summary.json')
...
PY
    ...
    if [[ $focus_rc -eq 42 ]]; then
        echo "[$(date +%T)] ==== crossing still collision=1.0 for all fresh97 focus candidates; skipping expensive selected eval ===="
        exit 0
  ...
  bash src/scripts/fresh97_selected_eval.sh
  echo "[$(date +%T)] ==== selected eval complete; parsing crossing summaries ===="
  /bin/python3 - <<'PY'
import json
from pathlib import Path

eval_dir = Path('/mnt/data/checkpoints/usv_rl/fresh97_selected_eval')
...
PY
  echo "[$(date +%T)] ==== fresh97 pipeline complete ===="
} 2>&1 | tee "$LOG"