#!/bin/bash
# fresh112: from fresh109 safe 2/3, learn final meters from raw navigation targets.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh112_raw_target_last5m.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh112_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh112_train.log}"
export RUN_NAME="${RUN_NAME:-fresh112}"
export TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-864}"
export BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-193}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --actor-log-std-init -3.95 \
  --force-actor-log-std -3.95 \
