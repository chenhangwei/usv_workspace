#!/bin/bash
# fresh101 crossing-only triage for progress-recovery candidates.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh101_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh101_crossing_focus_eval.log}
EPISODES=${EPISODES:-5}
STEPS=${STEPS:-360}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-360}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-96}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-32}
BASE_DOMAIN=${BASE_DOMAIN:-183}
FRESH101_LATEST_COUNT=${FRESH101_LATEST_COUNT:-20}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh101_crossing_progress_recover.pt

mapfile -t latest_fresh101 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh101_checkpoints/fresh101_crossing_progress_recover_step_*.pt 2>/dev/null | head -n "$FRESH101_LATEST_COUNT")
for model in "${latest_fresh101[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh101 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh101_focus_${label}.log"
...
rows.sort(key=lambda r: (r['collision_rate'], -r['success_rate'], r['timeout_rate'], -r['progress'], -r['worst_sep']))
summary = {'eval_dir': str(out), 'episodes': int(episodes), 'best': rows[0] if rows else None, 'has_crossing_breakthrough': any(r['collision_rate'] < 1.0 for r in rows), 'rows': rows}
summary_path = out / 'crossing_focus_summary.json'
summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
print('=== crossing_focus_summary ===')
for r in rows:
    print(f"coll={r['collision_rate']:.3f} succ={r['success_rate']:.3f} timeout={r['timeout_rate']:.3f} progress={r['progress']:.3f} sep={r['worst_sep']:.3f} ent={r['entanglement']:.3f} colregs={r['colregs']:.3f} omega={r['omega']:.1f} {r['label']}")
print(f"breakthrough={summary['has_crossing_breakthrough']}")
print(f"summary={summary_path}")
PY

echo "[$(date +%T)] fresh101 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"