#!/bin/bash
# fresh118 pipeline: train safe-gap lagging finish, then crossing triage baselines/final/checkpoints.
...
LOG="${LOG:-/tmp/fresh118_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh118_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh118_safe_gap_lagging_finish.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh118_checkpoints}"
...
{
  echo "========== fresh118 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh118.sh
  echo "========== fresh118 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh118_safe_gap_lagging_finish_step_*.pt 2>/dev/null || true)
  fi

  declare -A seen=()
  idx=0
  summary="$OUT/fresh118_crossing_summary.tsv"
...
print(
    f"fresh118 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh118 crossing summary =========="
  cat "$summary"
  echo "========== fresh118 pipeline complete =========="
