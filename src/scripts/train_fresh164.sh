#!/bin/bash
# fresh164: release priority give-way agents after the random encounter is deconflicted.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh164_priority_release_from_fresh163_step1152.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh164_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh164_train.log}"
RUN_NAME="${RUN_NAME:-fresh164}"
...
{
  echo "========== ${RUN_NAME} Priority Release (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-weight 3.80 \
  --random-deconflict-weight-end 2.80 \
  --random-deconflict-pretrain-epochs 80 \
  --random-deconflict-pretrain-learning-rate 5.0e-5 \
  --random-deconflict-pretrain-max-grad-norm 1.2 \
  ...
  --random-deconflict-time-horizon 38.0 \
  --random-deconflict-dcpa-target 1.95 \
  --random-deconflict-closing-speed-min 0.001 \
  --random-deconflict-local-danger \
  --random-deconflict-safe-separation 1.34 \
  --random-deconflict-release-separation 2.35 \
  --random-deconflict-power 0.75 \
  --random-deconflict-standon-speed 0.33 \
  --random-deconflict-yield-speed 0.18 \
  --random-deconflict-yield-danger-scale 0.55 \
  --random-deconflict-standon-omega 0.03 \
  --random-deconflict-yield-omega 0.26 \
  --random-deconflict-omega-weight 0.20 \
  --random-deconflict-standon-weight 0.30 \
  --random-deconflict-yield-weight 2.40 \
  --policy-anchor-weight 100.0 \
  --policy-anchor-weight-end 145.0 \
...
echo "${RUN_NAME} priority-release fit complete: $OUTPUT" | tee -a "$LOG_FILE"
