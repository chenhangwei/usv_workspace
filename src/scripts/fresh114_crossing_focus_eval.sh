...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh114_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh114_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-450}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-135}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-105}
BASE_DOMAIN=${BASE_DOMAIN:-146}
FRESH114_LATEST_COUNT=${FRESH114_LATEST_COUNT:-12}
...
if [[ "$INCLUDE_BASELINE" == "1" ]]; then
  add_candidate /mnt/data/checkpoints/usv_rl/fresh109_near_team_lagging_finish.pt
fi
add_candidate /mnt/data/checkpoints/usv_rl/fresh114_conservative_anchor_raw_target.pt

mapfile -t latest_fresh114 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh114_checkpoints/fresh114_conservative_anchor_raw_target_step_*.pt 2>/dev/null | head -n "$FRESH114_LATEST_COUNT")
for model in "${latest_fresh114[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh114 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh114_focus_${label}.log"
...
echo "[$(date +%T)] fresh114 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"