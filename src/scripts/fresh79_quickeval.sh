...
EVAL_DIR=/mnt/data/checkpoints/usv_rl/fresh79_eval
mkdir -p "$EVAL_DIR"

INTERP_DIR=/mnt/data/checkpoints/usv_rl/fresh79_interpolants
CANDIDATES=(
  /mnt/data/checkpoints/usv_rl/fresh74_best.pt
  /mnt/data/checkpoints/usv_rl/fresh78_best.pt
  /mnt/data/checkpoints/usv_rl/fresh76_headon_focus.pt
)
for f in "$INTERP_DIR"/*.pt; do
  CANDIDATES+=("$f")
done

LOG=/tmp/fresh79_quickeval.log
: > "$LOG"
...
for model in "${CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 180 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    --episode-timeout 70 --no-progress-timeout 20 \
    --output-json "$out" \
    ...
    > "/tmp/fresh79_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh79_quickeval_${base}.log)" | tee -a "$LOG"
  else
