#!/bin/bash
# fresh620: fresh619's recipe (identical CLI) PLUS a foundational reward-code
# upgrade: conflict_risk is now a PREDICTIVE DCPA/TCPA collision-risk index
# (CRI) instead of the old proximity x range_rate x forward_factor heuristic.
#
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh620_cpa_predictive_risk_3usv.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh620_checkpoints}"
...
echo "[fresh620] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh620] OUTPUT=$OUTPUT"
echo "[fresh620] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh620] PREDICTIVE conflict_risk (DCPA/TCPA CRI) + heading-convergence conflict-gated; fresh619 CLI unchanged"
