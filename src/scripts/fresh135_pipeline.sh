#!/bin/bash
# fresh135 pipeline: conservative fresh128 continuation and seeded triage.
...
LOG="${LOG:-/tmp/fresh135_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh135_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
EVAL_TIMEOUT="${EVAL_TIMEOUT:-520}"
EPISODE_TIMEOUT="${EPISODE_TIMEOUT:-135}"
NO_PROGRESS_TIMEOUT="${NO_PROGRESS_TIMEOUT:-105}"
BASE_DOMAIN="${BASE_DOMAIN:-212}"
MAX_DOMAIN="${MAX_DOMAIN:-232}"
SEED="${SEED:-1350}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh135_completion_polish_from_fresh128.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh135_checkpoints}"
...
FRESH134="${FRESH134:-/mnt/data/checkpoints/usv_rl/fresh134_finish_first_local_guard_from_fresh124_step0576.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh135 pipeline =========="
  echo "Started: $(date -Iseconds)"
  echo "Eval seed: $SEED"
  bash src/scripts/train_fresh135.sh
  echo "========== fresh135 train complete; paired crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH124" ]] && candidates+=("$FRESH124")
  [[ -f "$FRESH128" ]] && candidates+=("$FRESH128")
  [[ -f "$FRESH134" ]] && candidates+=("$FRESH134")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh135_completion_polish_from_fresh128_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh135_crossing_summary.tsv"
...
print(
    f"fresh135 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh135 crossing summary =========="
  cat "$summary"
  echo "========== fresh135 pipeline complete =========="
