#!/bin/bash
# fresh130 pipeline: train separation-floor polish and run crossing triage.
...
LOG="${LOG:-/tmp/fresh130_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh130_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh130_sep_floor_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh130_checkpoints}"
...
{
  echo "========== fresh130 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh130.sh
  echo "========== fresh130 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh130_sep_floor_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh130_crossing_summary.tsv"
...
print(
    f"fresh130 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh130 crossing summary =========="
  cat "$summary"
  echo "========== fresh130 pipeline complete =========="
