#!/bin/bash
# fresh602: plain speed_scale PPO with the speed-cap BUG FIX, 3-USV focus.
#
# THE FIX (in code, multi_agent_env.step): expand_policy_action was being fed the
# already-projected action, so the speed_scale throttle was double-processed
# (scale 1.0 -> clamped-as-linear 0.34 -> 0.34*v_nav) and the craft was capped at
# cruise^2 ~= 0.116 m/s. This was the REAL cause of the fresh600/601 "crawl" and
# their 0/11 gates -- NOT the speed_scale paradigm and NOT "the policy floors the
# throttle". Probe after the fix: scale=1.0 -> final speed 0.34, and the rule
# teacher (same env) now SOLVES crossing + clear_route closed-loop (goal=1.0,
# 0 collision), which no prior model achieved.
#
# This run is the clean A/B for that fix: identical to fresh601 (speed_scale,
# warm-start fresh520, same reward/net/PPO) EXCEPT
#   1. the code-level speed fix is now active, and
#   2. curriculum narrowed to the 3-USV scenarios (milestone discipline: prove
#      3-USV first; pentagram / 5-USV dense deferred to a later stage).
# If this passes the 3-USV repeated gate, the bug was the blocker and the
# paradigm works; teacher/BC becomes optional polish rather than a necessity.
#
# Warm-start from fresh520. Target: 150k steps.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh602_speedfix_3usv.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh602_checkpoints}"
...
echo "[fresh602] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh602] OUTPUT=$OUTPUT"
echo "[fresh602] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh602] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
echo "[fresh602] ACTION_MODE=speed_scale  SCALE_MIN=$ACTION_SPEED_SCALE_MIN  (3-USV focus, speed-fix active)"
