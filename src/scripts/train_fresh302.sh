#!/bin/bash
# fresh302: Stage-C overtaking recovery and head-on completion repair.
#
# Builds on fresh301.  The overtaking trace showed both vessels drifting to the
# CTE clip while the rear vessel stayed fast and close.  This run teaches:
#   - front vessel may keep route even with a rear neighbor,
#   - rear vessel must open a starboard corridor before passing,
#   - close same-lane speed is coupled to separation.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh302_overtaking_repair_from_fresh301.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh302_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh302_train.log}"
RUN_NAME="${RUN_NAME:-fresh302}"
...
FRESH302_ARGS="
  ...
  --scenario-spawn-position-std 0.04
  --scenario-spawn-heading-std 0.018
  --scenario-goal-position-std 0.03
  --random-encounter-route-priority
  --force-actor-log-std -2.32
  ...
  --speed-distance-coupling-penalty-weight 1.50
  --speed-distance-coupling-threshold 2.40
  --proximity-gradient-penalty-weight 0.55
  --proximity-gradient-distance 1.80
  --conflict-overspeed-penalty-weight 0.65
  --overtaking-starboard-turn-reward-weight 0.95
  --overtaking-forward-reward-weight 0.36
  --overtaking-corridor-reward-weight 1.70
  --overtaking-centerline-penalty-weight 1.80
  --overtaking-close-penalty-weight 2.20
  --head-on-corridor-reward-weight 2.8
  --head-on-centerline-penalty-weight 3.0
  --head-on-turn-reward-weight 1.2
  --head-on-forward-reward-weight 1.25
  --head-on-close-penalty-weight 2.4
  --head-on-no-turn-penalty-weight 1.5
  ...
  --random-clear-ahead-scenario two_usv_overtaking
  ...
  --random-deconflict-yield-weight 5.00
  --random-deconflict-pretrain-epochs 1
  --random-deconflict-pretrain-learning-rate 3.0e-7
  --random-deconflict-pretrain-max-grad-norm 0.030
  --random-role-balance-weight 4.40
  --random-role-balance-weight-end 4.90
  --random-role-balance-pretrain-epochs 1
  --random-role-balance-pretrain-learning-rate 2.5e-7
  ...
  --random-role-balance-yield-weight 2.25
  --random-role-balance-omega-weight 0.38
