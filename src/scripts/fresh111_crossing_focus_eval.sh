...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh111_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh111_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-450}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-135}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-105}
BASE_DOMAIN=${BASE_DOMAIN:-134}
FRESH111_LATEST_COUNT=${FRESH111_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh111_safe_last5m_finish.pt

mapfile -t latest_fresh111 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh111_checkpoints/fresh111_safe_last5m_finish_step_*.pt 2>/dev/null | head -n "$FRESH111_LATEST_COUNT")
for model in "${latest_fresh111[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh111 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh111_focus_${label}.log"
...
echo "[$(date +%T)] fresh111 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"