#!/bin/bash
# fresh175: release lagging low-priority finish using local, not fleet-global, clearance scaling.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh175_local_safe_finish_from_fresh172_step1260.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh175_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh175_train.log}"
RUN_NAME="${RUN_NAME:-fresh175}"
...
{
  echo "========== ${RUN_NAME} Local Safe-Finish Recovery (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-safe-finish-low-priority-speed-multiplier 1.35 \
  --random-safe-finish-max-omega 0.070 \
...
echo "${RUN_NAME} local safe-finish recovery complete: $OUTPUT" | tee -a "$LOG_FILE"
