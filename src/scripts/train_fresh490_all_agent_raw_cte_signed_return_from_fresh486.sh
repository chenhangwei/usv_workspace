#!/bin/bash
# fresh490: all-agent raw-CTE signed-return repair from fresh486.
#
# fresh489 fixed the original seed1460/usv_02 raw CTE runaway, but the worst CTE
# shifted to agents that were excluded from the recovery teacher.  This branch
# applies the same signed raw-CTE recovery gate to all agents while keeping the
# clear-ahead push scoped to agent 1 to avoid thinning the close-pair margin.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh490_all_agent_raw_cte_signed_return_from_fresh486.pt}" \
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh490_checkpoints}" \
...
exec bash scripts/train_fresh464_goal_heading_action_probe.sh \
  ...
  --random-cte-recovery-all-agents \
