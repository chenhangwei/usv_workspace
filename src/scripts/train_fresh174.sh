#!/bin/bash
# fresh174: let high-progress/high-CTE off-route recovery overlap deconflict threats.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh174_overlap_offroute_from_fresh172_step1260.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh174_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh174_train.log}"
RUN_NAME="${RUN_NAME:-fresh174}"
...
{
  echo "========== ${RUN_NAME} Threat-Overlap Off-Route Recovery (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-weight-end 4.55 \
  ...
  --random-offroute-finish-weight 5.80 \
  ...
  --random-offroute-finish-min-neighbor-separation 1.12 \
  --random-offroute-finish-max-cpa-score 0.0 \
  --random-offroute-finish-max-local-score 0.0 \
  --random-offroute-finish-allow-threat-overlap \
  --random-offroute-finish-target-speed 0.08 \
  --random-offroute-finish-min-speed 0.025 \
  --random-offroute-finish-max-omega 0.30 \
  --random-offroute-finish-omega-reference 0.50 \
  --random-offroute-finish-omega-weight 1.20 \
...
echo "${RUN_NAME} threat-overlap off-route recovery complete: $OUTPUT" | tee -a "$LOG_FILE"
