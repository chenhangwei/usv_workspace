#!/bin/bash
# fresh111: safe 2/3 continuation from fresh109, only nudging the final ~5m.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh111_safe_last5m_finish.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh111_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh111_train.log}"
export RUN_NAME="${RUN_NAME:-fresh111}"
export TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-1152}"
export BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-199}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --actor-log-std-init -3.85 \
  --force-actor-log-std -3.85 \
  ...
  --lagging-finish-weight 4.5 \
  --lagging-finish-weight-end 3.5 \
  --lagging-finish-distance 5.2 \
  --lagging-finish-goal-tolerance 0.8 \
  --lagging-finish-phase-min -1.0 \
  --lagging-finish-target-speed 0.15 \
  --lagging-finish-min-speed-scale 0.50 \
