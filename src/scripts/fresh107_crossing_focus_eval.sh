#!/bin/bash
# fresh107 crossing-only triage for team-state gated lagging-finish candidates.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh107_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh107_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-450}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-135}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-90}
BASE_DOMAIN=${BASE_DOMAIN:-174}
FRESH107_LATEST_COUNT=${FRESH107_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh107_lagging_finish.pt

mapfile -t latest_fresh107 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh107_checkpoints/fresh107_lagging_finish_step_*.pt 2>/dev/null | head -n "$FRESH107_LATEST_COUNT")
for model in "${latest_fresh107[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh107 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh107_focus_${label}.log"
  domain=$((BASE_DOMAIN + (idx % 48)))
  idx=$((idx + 1))
...
echo "[$(date +%T)] fresh107 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"