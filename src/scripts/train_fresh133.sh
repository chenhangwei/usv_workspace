#!/bin/bash
# fresh133: local-danger neighbor brake from fresh124 step0576.
#
# fresh132 localized activation with require-neighbor, but the linear-speed
# danger was still scaled by global team-min separation.  This run enables the
# trainer's local-danger brake so each active agent scales by its own nearest
# neighbor distance, then applies a modestly stronger head-only update.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh133_local_danger_from_fresh124_step0576.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh133_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh133_train.log}"
export RUN_NAME="${RUN_NAME:-fresh133}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --team-safety-brake-weight 0.24 \
  --team-safety-brake-weight-end 0.10 \
  ...
  --team-safety-brake-release-separation 2.85 \
  --team-safety-brake-target-speed 0.20 \
  --team-safety-brake-omega-weight 0.012 \
  ...
  --team-safety-brake-power 2.10 \
  --team-safety-brake-crossing-only \
  --policy-anchor-weight 360.0 \
  --policy-anchor-weight-end 480.0 \
