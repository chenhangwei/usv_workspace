#!/bin/bash
# fresh98 pipeline: train pairwise-conflict model, run crossing focus eval, then full selected eval only after a crossing breakthrough.
...
LOG="${LOG:-/tmp/fresh98_pipeline.log}"
export PYTHONUNBUFFERED=1

{
  echo "[$(date +%T)] ==== fresh98 pipeline start ===="
  echo "[$(date +%T)] Train log: ${LOG_FILE:-/tmp/fresh98_train.log}"
  bash src/scripts/train_fresh98.sh
  echo "[$(date +%T)] ==== training complete; running crossing-focus eval ===="
  bash src/scripts/fresh98_crossing_focus_eval.sh
...
summary_path = Path('/mnt/data/checkpoints/usv_rl/fresh98_crossing_focus_eval/crossing_focus_summary.json')
...
PY
  ...
  if [[ $focus_rc -eq 42 ]]; then
    echo "[$(date +%T)] ==== crossing still collision=1.0 for all fresh98 focus candidates; skipping expensive selected eval ===="
    exit 0
  ...
  bash src/scripts/fresh98_selected_eval.sh
  echo "[$(date +%T)] ==== selected eval complete; parsing crossing summaries ===="
  /bin/python3 - <<'PY'
import json
from pathlib import Path

eval_dir = Path('/mnt/data/checkpoints/usv_rl/fresh98_selected_eval')
...
PY
  echo "[$(date +%T)] ==== fresh98 pipeline complete ===="
} 2>&1 | tee "$LOG"
