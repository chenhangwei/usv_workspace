#!/bin/bash
# Fixed-seed random-only screening for fresh168.
...
OUT="${OUT:-/tmp/fresh168_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh168_random_hold_lowprio_from_fresh163_step1152.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh168_checkpoints/fresh168_random_hold_lowprio_from_fresh163_step1152_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh168 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh168-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
