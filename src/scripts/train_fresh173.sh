#!/bin/bash
# fresh173: local-only off-route recovery from the fresh172 step1260 candidate.
...
FALLBACK_INPUT="${FALLBACK_INPUT:-/mnt/data/checkpoints/usv_rl/fresh172_offroute_recovery_from_fresh171.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh173_local_offroute_from_fresh172_step1260.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh173_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh173_train.log}"
RUN_NAME="${RUN_NAME:-fresh173}"
...
if [[ ! -f "$BASE_INPUT" && -f "$FALLBACK_INPUT" ]]; then
  echo "WARN: fresh172 step1260 base missing, falling back to fresh172 final: $FALLBACK_INPUT"
  BASE_INPUT="$FALLBACK_INPUT"
fi
if [[ -z "$BASE_INPUT" || ! -f "$BASE_INPUT" ]]; then
  echo "ERROR: base checkpoint not found. Set BASE_INPUT or finish fresh172 first."
  exit 1
fi
...
{
  echo "========== ${RUN_NAME} Local Off-Route Recovery (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-safe-finish-min-team-separation 2.60 \
  --random-safe-finish-full-team-separation 3.20 \
  --random-safe-finish-min-neighbor-separation 2.78 \
  ...
  --random-safe-finish-low-priority-speed-multiplier 1.20 \
  --random-safe-finish-max-omega 0.055 \
  --random-safe-finish-omega-weight 0.05 \
  --random-goal-hold-weight 0.00 \
  --random-goal-hold-weight-end 0.00 \
  --random-goal-hold-distance 0.0 \
  --random-offroute-finish-weight 5.20 \
  --random-offroute-finish-weight-end 6.40 \
  --random-offroute-finish-goal-tolerance 0.8 \
  --random-offroute-finish-max-distance 13.0 \
  --random-offroute-finish-min-distance 1.00 \
  --random-offroute-finish-route-progress-min 0.76 \
  --random-offroute-finish-min-abs-cte 0.85 \
  --random-offroute-finish-full-abs-cte 3.00 \
  --random-offroute-finish-phase-min -1.0 \
  --random-offroute-finish-min-team-separation 0.00 \
  --random-offroute-finish-min-neighbor-separation 2.25 \
  --random-offroute-finish-max-cpa-score 0.0 \
  --random-offroute-finish-max-local-score 0.0 \
  --random-offroute-finish-target-speed 0.09 \
  --random-offroute-finish-min-speed 0.030 \
  --random-offroute-finish-max-omega 0.28 \
  --random-offroute-finish-omega-reference 0.50 \
  --random-offroute-finish-omega-weight 1.10 \
  --policy-anchor-weight 165.0 \
  --policy-anchor-weight-end 215.0 \
...
echo "${RUN_NAME} local off-route recovery complete: $OUTPUT" | tee -a "$LOG_FILE"
