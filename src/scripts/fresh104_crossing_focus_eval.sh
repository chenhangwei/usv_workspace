#!/bin/bash
# fresh104 crossing-only triage for goal-hold near-goal candidates.
...
LOG=${LOG:-/tmp/fresh104_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-380}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-420}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-120}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-62}
...
if [[ "$INCLUDE_BASELINE" == "1" ]]; then
  add_candidate /mnt/data/checkpoints/usv_rl/fresh102_checkpoints/fresh102_post_conflict_finish_step_0001728.pt
  add_candidate /mnt/data/checkpoints/usv_rl/fresh103_checkpoints/fresh103_near_goal_finish_step_0002880.pt
fi
add_candidate /mnt/data/checkpoints/usv_rl/fresh104_goal_hold.pt

mapfile -t latest_fresh104 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh104_checkpoints/fresh104_goal_hold_step_*.pt 2>/dev/null | head -n "$FRESH104_LATEST_COUNT")
for model in "${latest_fresh104[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh104 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh104_focus_${label}.log"
...
echo "[$(date +%T)] fresh104 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"
