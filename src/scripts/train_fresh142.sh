#!/bin/bash
# fresh142: close-margin finish/route-keeping continuation from fresh140.
#
# User target:
...
#   - strengthen random 2/3-USV crossing/disengagement without degrading the
#     stable three-USV runtime branch
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh142_close_margin_finish_route_from_fresh140.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh142_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh142_train.log}"
RUN_NAME="${RUN_NAME:-fresh142}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-1152}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-184}"
...
{
  echo "========== ${RUN_NAME} Close-Margin Finish Route-Keeping (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --near-miss-distance 1.12 \
  --near-miss-weight 245.0 \
  --near-miss-exponent 2.55 \
  --head-on-near-miss-distance 1.26 \
  ...
  --proximity-gradient-distance 1.26 \
  --speed-distance-coupling-penalty-weight 16.0 \
  --speed-distance-coupling-threshold 1.22 \
  ...
  --path-deviation-penalty-weight 3.4 \
  --path-deviation-tolerance 0.58 \
  --path-deviation-conflict-scale 0.70 \
  --cte-clip-range 4.0 \
  ...
  --near-goal-finish-weight 2.80 \
  --near-goal-finish-weight-end 1.20 \
  ...
  --policy-anchor-weight 980.0 \
  --policy-anchor-weight-end 1300.0 \
  ...
  --scenario-spawn-heading-std 0.030 \
  --scenario-goal-position-std 0.06 \
...
echo "${RUN_NAME} close-margin finish route-keeping complete: $OUTPUT" | tee -a "$LOG_FILE"
