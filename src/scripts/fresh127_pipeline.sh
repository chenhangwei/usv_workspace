#!/bin/bash
# fresh127 pipeline: conservative fresh124_step0576 continuation and triage.
...
LOG="${LOG:-/tmp/fresh127_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh127_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh127_micro_finish_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh127_checkpoints}"
...
{
  echo "========== fresh127 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh127.sh
  echo "========== fresh127 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh127_micro_finish_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh127_crossing_summary.tsv"
...
print(
    f"fresh127 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh127 crossing summary =========="
  cat "$summary"
  echo "========== fresh127 pipeline complete =========="
