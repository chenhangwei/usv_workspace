#!/bin/bash
# fresh151: random-only local safety brake on top of fresh150.
#
# Direction:
#   - keep fresh150/fresh142 crossing behavior anchored
#   - stop treating DCPA deficit as a broad all-scenario pressure
#   - add a short-range random-encounter auxiliary that brakes and turns away
#     only when the nearest neighbor is inside the final safety band
...
PREFERRED_BASE="/mnt/data/checkpoints/usv_rl/fresh150_cpa_margin_soft_clear_from_fresh142.pt"
FALLBACK_BASE="/mnt/data/checkpoints/usv_rl/fresh142_close_margin_finish_route_from_fresh140.pt"
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh151_random_safety_gate_from_fresh150.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh151_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh151_train.log}"
RUN_NAME="${RUN_NAME:-fresh151}"
...
{
  echo "========== ${RUN_NAME} Random Safety Gate (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --proximity-gradient-penalty-weight 9.6 \
  --proximity-gradient-distance 1.42 \
  --speed-distance-coupling-penalty-weight 15.0 \
  --speed-distance-coupling-threshold 1.42 \
  --separation-recovery-weight 1.8 \
  --avoidance-turn-reward-weight 3.4 \
  --anticipatory-avoidance-turn-reward-weight 4.8 \
  --anticipatory-avoidance-turn-penalty-weight 7.8 \
  --anticipatory-avoidance-turn-distance 3.2 \
  --anticipatory-cpa-distance 3.2 \
  --anticipatory-cpa-time-horizon 5.2 \
  --anticipatory-dcpa-target 1.00 \
  --anticipatory-dcpa-deficit-penalty-weight 2.0 \
  --anticipatory-dcpa-improvement-reward-weight 9.0 \
  --anticipatory-closing-reduction-reward-weight 8.5 \
  --anticipatory-yield-speed 0.075 \
  --anticipatory-yield-speed-penalty-weight 5.0 \
  ...
  --crossing-imitation-weight 0.014 \
  ...
  --team-safety-brake-weight-end 0.22 \
  ...
  --team-safety-brake-safe-separation 0.82 \
  --team-safety-brake-release-separation 1.75 \
  --team-safety-brake-target-speed 0.06 \
  --team-safety-brake-omega-weight 0.35 \
  --team-safety-brake-target-omega 0.18 \
  --team-safety-brake-turn-mode away \
  --team-safety-brake-require-neighbor \
  --team-safety-brake-local-danger \
  --team-safety-brake-power 1.10 \
  --team-safety-brake-random-only \
  --policy-anchor-weight 1150.0 \
  --policy-anchor-weight-end 1450.0 \
...
echo "${RUN_NAME} random safety gate complete: $OUTPUT" | tee -a "$LOG_FILE"
