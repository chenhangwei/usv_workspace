#!/bin/bash
# Three-episode confirm for the fresh152 final checkpoint.
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh152_random_first_safety_brake_from_fresh151.pt}"
LABEL="${LABEL:-fresh152_random_first_safety_brake_from_fresh151}"
SUMMARY="${SUMMARY:-$OUT/fresh152_confirm_summary.tsv}"
EPISODES="${EPISODES:-3}"
STEPS="${STEPS:-420}"
...
print(
    f"fresh152-confirm {label} {scenario_name}: episodes={row['episodes']} "
    f"coll={row['collision']:.3f} succ={row['success']:.3f} "
    f"timeout={row['timeout']:.3f} progress={row['progress']:.3f} "
    f"goal={row['goal']:.3f} sep={row['sep']:.3f} cte={row['cte']:.3f} "
    f"ent={row['entanglement']:.3f} colregs={row['colregs']:.3f} "
...
printf '========== fresh152 confirm summary =========='"\n"
cat "$SUMMARY"