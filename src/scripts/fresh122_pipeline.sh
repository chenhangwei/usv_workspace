#!/bin/bash
# fresh122 pipeline: train early brake plus escape turn, then crossing triage.
...
LOG="${LOG:-/tmp/fresh122_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh122_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh122_early_brake_escape_turn.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh122_checkpoints}"
FRESH109="${FRESH109:-/mnt/data/checkpoints/usv_rl/fresh109_near_team_lagging_finish.pt}"
FRESH121="${FRESH121:-/mnt/data/checkpoints/usv_rl/fresh121_early_team_brake.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh122 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh122.sh
  echo "========== fresh122 train complete; crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH109" ]] && candidates+=("$FRESH109")
  [[ -f "$FRESH121" ]] && candidates+=("$FRESH121")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh122_early_brake_escape_turn_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh122_crossing_summary.tsv"
...
print(
    f"fresh122 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh122 crossing summary =========="
  cat "$summary"
  echo "========== fresh122 pipeline complete =========="
