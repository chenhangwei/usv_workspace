#!/bin/bash
# fresh622: BALANCED-CURRICULUM consolidation run. User requirements (2026-07-03):
#   1. Reach waypoints EFFECTIVELY -- no detours, no circling.
#   2. When no avoidance is needed: STRAIGHT lines, no S-oscillation.
#   3. When no avoidance is needed: hug the ORIGINAL route (low CTE).
#   4. Dense starts must NOT tangle -- boats disperse fast to their own goals.
#   5. Entanglement is ABSOLUTELY forbidden.
#
# WHY A NEW RUN (whack-a-mole evidence from the 36-ep offline gates):
#   model     head_on  crossing  overtaking  cluster reach   verdict
#   fresh616    0/3      0/3        0/3        1/9           safe but can't finish
#   fresh620    0/3      3/3        3/3        9/9           crossing/overtaking broke
#   fresh621    3/3      0/3        3/3        9/9 (7<=0.5m) crossing FIXED, head_on forgot
# fresh621's head_on collision was reconstructed frame-by-frame: conflict_risk
# and clear_ahead behaved CORRECTLY (risk 0.30@5.7m, clear_ahead off) but the
# policy simply never turned -- pure CATASTROPHIC FORGETTING, because
# two_usv_head_on was NEVER in any curriculum (0/13 slots) and overtaking had
# only 1/13, while two generations of warm-start (400k steps) drifted away from
# fresh616's avoidance. The reward machinery is now proven right (crossing got
# fixed by the CPA early-predict knobs); what's broken is EXPOSURE BALANCE.
#
# THE fresh622 CHANGES vs fresh621:
#   (A) WARM-START back to fresh616 (the only 0/9-collision base). The anchor
#       therefore pulls toward robust avoidance while the reward knobs add
#       terminal capture + early predictive avoidance ON TOP, in ONE run
#       instead of stacked drifting generations.
#   (B) BALANCED curriculum (15 slots) -- every COLREGS encounter type gets
#       dedicated rounds so nothing can silently rot (fixes forgetting):
#       solo x2, waypoint x2, clear_route x2       (reqs 1-3: route/straight)
#       cluster_escape x2                          (req 4: dense-start dispersal)
#       two_usv_head_on x2  <- NEW, was 0 slots    (head_on forgot in 621)
#       three_usv_crossing x2                      (keep 621's fix trained)
#       three_usv_overtaking x2 <- was 1 slot      (broke in 620/621)
#       three_usv_random_encounter x1              (generalization glue)
#   (B2) TRAINING VOLUME DOUBLED (user directive 2026-07-03: "每个场景的训练量
#       一定要足够，绝对不允许遗忘"): TOTAL_TIMESTEPS 200k -> 400k. At 400k/15
#       slots each scenario type gets ~27k steps (~100-250 episodes), roughly
#       2x the per-scenario exposure of any previous run (and head_on goes
#       from ZERO curriculum exposure ever to ~53k steps across its 2 slots).
#       LR/entropy/anchor schedules stretch automatically (they decay over
#       total_timesteps), so the anchor keeps guarding fresh616's avoidance
#       for the WHOLE longer run. Expected wall time ~30-32 h. Mid-run
#       checkpoints allow gate checks at ~130k/260k to VERIFY (not assume)
#       nothing is forgotten before the run finishes.
#   (C) ANTI-ENTANGLEMENT explicit (req 5), knobs existed but were 0:
#       --entanglement-penalty-weight 1.0  --entanglement-distance 2.0
#       --entanglement-grace-steps 25      (~5 s inside 2 m before ramp)
#       --entanglement-low-speed-penalty-weight 0.5 (no orbital stalling:
#         if stuck close, DRIVE OUT, don't circle in place)
#       --separation-recovery-weight 0.5   (positive reward for actively
#         opening distance inside the near-miss band -> fast disengage)
#   (D) KEEP fresh621's proven early-predictive-avoid knobs verbatim:
#       CRI horizon 15 s, safe DCPA 1.5 m, head-on/crossing guidance 7.5 m,
#       plus all terminal-capture/anti-circling terms (clear-ahead trio,
#       conflict-gated heading-convergence, turn-speed coupling etc).
#
# fresh621 provenance (kept): fresh620 recipe PLUS "look further ahead / avoid
# earlier" tuning of the predictive CRI, after fresh620's offline gate (36 ep)
# showed head_on FIXED (0/3 collisions, was 3/3) and cluster_escape best-yet
# (9/9 reach, 5/9<=0.5m), but three_usv_crossing STILL 3/3 collided and
# three_usv_overtaking regressed to 3/3.
#
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh622_balanced_no_entangle_3usv.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh622_checkpoints}"
...
echo "[fresh622] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh622] OUTPUT=$OUTPUT"
echo "[fresh622] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh622] BALANCED curriculum 15 slots (head_on x2 NEW, overtaking x2, random x1) + anti-entanglement + fresh621 early-predict knobs; 400k steps (2x per-scenario volume, no forgetting)"
