...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh324_conservative_event_guard_from_fresh321.pt}"
...
ANCHOR_WEIGHT="${ANCHOR_WEIGHT:-40.0}"
DECONF_WEIGHT="${DECONF_WEIGHT:-0.10}"
RISK_GUARD_WEIGHT="${RISK_GUARD_WEIGHT:-2.20}"
MIN_ERROR_NORM="${MIN_ERROR_NORM:-0.020}"
MAX_GRAD_NORM="${MAX_GRAD_NORM:-0.10}"
RISK_GUARD_MAX_LINEAR="${RISK_GUARD_MAX_LINEAR:-0.100}"

if [[ ! -f "$SOURCE_MODEL" ]]; then
  echo "missing source model: $SOURCE_MODEL" >&2
  exit 1
fi
for trace in "$TRACE_D104" "$TRACE_D105"; do
  if [[ ! -f "$trace" ]]; then
