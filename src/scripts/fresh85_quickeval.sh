...
EVAL_DIR=/mnt/data/checkpoints/usv_rl/fresh85_full5_eval
...
for idx in "${!CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 240 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/fresh85_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh85_quickeval_${base}.log)" | tee -a "$LOG"
  else
