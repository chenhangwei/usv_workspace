#!/bin/bash
# Three-episode confirm for a selected fresh156 checkpoint.
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh156_yield_guard_long_finish_from_fresh153.pt}"
LABEL="${LABEL:-$(basename "$MODEL" .pt)}"
SUMMARY="${SUMMARY:-$OUT/fresh156_confirm_summary.tsv}"
EPISODES="${EPISODES:-3}"
STEPS="${STEPS:-420}"
...
print(
    f"fresh156-confirm {label} {scenario_name}: episodes={row['episodes']} "
...
printf '========== fresh156 confirm summary =========='"\n"
cat "$SUMMARY"