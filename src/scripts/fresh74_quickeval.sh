#!/bin/bash
# fresh74 quick_eval — 全 5 场景评估 (即使 fresh74 只训 crossing,
# 也必须验证 random_encounter / overtaking 是否退化)
set -o pipefail
...
CANDIDATES=(
  /mnt/data/checkpoints/usv_rl/fresh73_best.pt
  /mnt/data/checkpoints/usv_rl/fresh74_crossing.pt
)
for f in "$CKPT_DIR"/fresh74_crossing_step_*.pt; do
  CANDIDATES+=("$f")
done

LOG=/tmp/fresh74_quickeval.log
: > "$LOG"
...
for model in "${CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 180 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/fresh74_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh74_quickeval_${base}.log)" | tee -a "$LOG"
  else
