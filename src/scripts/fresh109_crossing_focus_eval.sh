...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh109_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh109_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-400}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-450}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-135}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-100}
BASE_DOMAIN=${BASE_DOMAIN:-158}
FRESH109_LATEST_COUNT=${FRESH109_LATEST_COUNT:-12}
...
mapfile -t latest_fresh109 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh109_checkpoints/fresh109_near_team_lagging_finish_step_*.pt 2>/dev/null | head -n "$FRESH109_LATEST_COUNT")
for model in "${latest_fresh109[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh109 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh109_focus_${label}.log"
...
echo "[$(date +%T)] fresh109 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"