#!/bin/bash
# fresh78 quick_eval — full 5-scenario eval for fresh78 crossing repair run
set -o pipefail
...
CANDIDATES=(
  ...
  /mnt/data/checkpoints/usv_rl/fresh78_crossing_repair.pt
)
for f in "$CKPT_DIR"/fresh78_crossing_repair_step_*.pt; do
  CANDIDATES+=("$f")
done

LOG=/tmp/fresh78_quickeval.log
: > "$LOG"
...
for model in "${CANDIDATES[@]}"; do
  ...
  ROS_DOMAIN_ID=$domain timeout 180 python3 -u -m usv_rl.evaluate_mappo_policy \
    ...
    > "/tmp/fresh78_quickeval_${base}.log" 2>&1
  rc=$?
  if [[ $rc -ne 0 || ! -f "$out" ]]; then
    echo "[$(date +%T)]   FAIL $base rc=$rc (see /tmp/fresh78_quickeval_${base}.log)" | tee -a "$LOG"
  else
