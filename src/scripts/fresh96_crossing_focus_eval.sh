#!/bin/bash
# fresh96 crossing-only quick triage for deciding whether to continue to fresh97.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh96_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh96_crossing_focus_eval.log}
EPISODES=${EPISODES:-5}
STEPS=${STEPS:-320}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-260}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-82}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-24}
BASE_DOMAIN=${BASE_DOMAIN:-181}
LATEST_COUNT=${LATEST_COUNT:-10}
...
mapfile -t fresh96_latest < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh96_checkpoints/fresh96_crossing_eta_priority_repair_step_*.pt 2>/dev/null | head -n "$LATEST_COUNT")
for model in "${fresh96_latest[@]}"; do
  add_candidate "$model"
done
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh96_focus_${label}.log"
...
summary = {'eval_dir': str(out), 'episodes': int(episodes), 'rows': rows, 'best': rows[0] if rows else None}
...
echo "[$(date +%T)] fresh96 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"
