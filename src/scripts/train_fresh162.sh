#!/bin/bash
# fresh162: observable-priority random deconflict.
#
# fresh161 proved that direct actor fitting can remove the fixed-seed random
# collision, but hidden agent-index roles made the actor over-brake. This run
# uses the observable crossing_priority feature as the role signal: low-priority
# boats yield, high-priority boats keep moving.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh162_priority_random_fit_from_fresh159_step1728.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh162_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh162_train.log}"
RUN_NAME="${RUN_NAME:-fresh162}"
...
{
  echo "========== ${RUN_NAME} Priority Random Fit (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-safe-separation 1.35 \
  --random-deconflict-release-separation 2.55 \
  --random-deconflict-power 0.65 \
  --random-deconflict-standon-speed 0.33 \
  --random-deconflict-yield-speed 0.08 \
  --random-deconflict-standon-omega 0.04 \
  --random-deconflict-yield-omega 0.28 \
  --random-deconflict-omega-weight 0.25 \
  --random-deconflict-standon-weight 0.55 \
  --random-deconflict-yield-weight 3.20 \
...
echo "${RUN_NAME} priority random fit complete: $OUTPUT" | tee -a "$LOG_FILE"
