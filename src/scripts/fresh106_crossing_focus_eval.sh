#!/bin/bash
# fresh106 crossing-only triage for narrow post-conflict finish candidates.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh106_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh106_crossing_focus_eval.log}
EPISODES=${EPISODES:-3}
STEPS=${STEPS:-380}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-420}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-130}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-82}
BASE_DOMAIN=${BASE_DOMAIN:-187}
FRESH106_LATEST_COUNT=${FRESH106_LATEST_COUNT:-12}
...
add_candidate /mnt/data/checkpoints/usv_rl/fresh106_post_conflict_narrow_finish.pt

mapfile -t latest_fresh106 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh106_checkpoints/fresh106_post_conflict_narrow_finish_step_*.pt 2>/dev/null | head -n "$FRESH106_LATEST_COUNT")
for model in "${latest_fresh106[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh106 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh106_focus_${label}.log"
...
echo "[$(date +%T)] fresh106 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"