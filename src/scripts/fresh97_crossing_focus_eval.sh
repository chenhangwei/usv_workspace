#!/bin/bash
# fresh97 crossing-only triage. Run this before expensive full selected eval.
...
OUT=${OUT:-/mnt/data/checkpoints/usv_rl/fresh97_crossing_focus_eval}
LOG=${LOG:-/tmp/fresh97_crossing_focus_eval.log}
EPISODES=${EPISODES:-5}
STEPS=${STEPS:-340}
EVAL_TIMEOUT=${EVAL_TIMEOUT:-300}
EPISODE_TIMEOUT=${EPISODE_TIMEOUT:-88}
NO_PROGRESS_TIMEOUT=${NO_PROGRESS_TIMEOUT:-28}
BASE_DOMAIN=${BASE_DOMAIN:-171}
FRESH97_LATEST_COUNT=${FRESH97_LATEST_COUNT:-12}
FRESH96_LATEST_COUNT=${FRESH96_LATEST_COUNT:-0}
...
if (( FRESH96_LATEST_COUNT > 0 )); then
...
mapfile -t latest_fresh97 < <(ls -1t /mnt/data/checkpoints/usv_rl/fresh97_checkpoints/fresh97_unfrozen_eta_temporal_repair_step_*.pt 2>/dev/null | head -n "$FRESH97_LATEST_COUNT")
for model in "${latest_fresh97[@]}"; do
  add_candidate "$model"
done

if [[ ${#CANDIDATES[@]} -eq 0 ]]; then
  echo "No fresh97 crossing-focus candidates found." | tee -a "$LOG"
  exit 1
fi
...
for model in "${CANDIDATES[@]}"; do
  ...
  eval_log="/tmp/fresh97_focus_${label}.log"
...
echo "[$(date +%T)] fresh97 crossing focus eval complete: $OUT/crossing_focus_summary.json" | tee -a "$LOG"
