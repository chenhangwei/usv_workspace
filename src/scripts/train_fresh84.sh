#!/bin/bash
# fresh84: scenario-specific overtaking residual branch from fresh79_best
#
# fresh83 postmortem:
#   - encounter-conditioned residual can move the policy along the same collision-vs-timeout frontier,
#     but it leaks into random_encounter because the branch fires on local geometry, not on the episode scenario.
#   - the next discriminating step is to keep the shared actor frozen, but route residual capacity by scenario id
#     so overtaking corrections stay overtaking-only.
#
# working hypothesis:
#   A scenario-specific residual branch, trained only on three_usv_overtaking, can preserve fresh79 behavior in
#   random_encounter while still learning the local overtaking correction that evaluator-time router/blend could not.
...
OUTPUT="/mnt/data/checkpoints/usv_rl/fresh84_overtaking_scenario_residual.pt"
CKPT_DIR="/mnt/data/checkpoints/usv_rl/fresh84_checkpoints"
...
echo "========== fresh84 Overtaking Scenario Residual (8K from fresh79_best) =========="
python3 -m usv_rl.train_mappo_policy \
  ...
  --base-ros-domain-id 206

echo "fresh84 overtaking scenario residual complete: $OUTPUT"
