...
WARMSTART_FROM="${WARMSTART_FROM:-/mnt/data/checkpoints/usv_rl/fresh518_goalhold_excl.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh519_returnroute.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh519_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-260}"

echo "[fresh519] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh519] OUTPUT=$OUTPUT"
echo "[fresh519] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh519] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
...
exec python3 -m usv_rl.train_mappo_policy \
  ...
  \
  --path-deviation-penalty-weight 0.35 \
