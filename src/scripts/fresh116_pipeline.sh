#!/bin/bash
# fresh116 pipeline: train linear-only hold/finish repair, then crossing eval final and checkpoints.
...
LOG="${LOG:-/tmp/fresh116_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh116_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
EVAL_TIMEOUT="${EVAL_TIMEOUT:-450}"
EPISODE_TIMEOUT="${EPISODE_TIMEOUT:-135}"
NO_PROGRESS_TIMEOUT="${NO_PROGRESS_TIMEOUT:-105}"
BASE_DOMAIN="${BASE_DOMAIN:-168}"
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh116_linear_hold_finish.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh116_checkpoints}"
...
{
  echo "========== fresh116 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh116.sh
  echo "========== fresh116 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh116_linear_hold_finish_step_*.pt 2>/dev/null || true)
  fi

  declare -A seen=()
  idx=0
  summary="$OUT/fresh116_crossing_summary.tsv"
...
print(
    f"fresh116 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh116 crossing summary =========="
  cat "$summary"
  echo "========== fresh116 pipeline complete =========="
