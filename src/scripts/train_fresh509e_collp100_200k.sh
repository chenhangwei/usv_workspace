...
WARMSTART_FROM="${WARMSTART_FROM:-/mnt/data/checkpoints/usv_rl/fresh509a3_checkpoints/fresh509a3_criticonly_step_0032768.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh509e_collp100_200k.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh509e_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-236}"

echo "[fresh509e] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh509e] OUTPUT=$OUTPUT"
echo "[fresh509e] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh509e] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
...
exec python3 -m usv_rl.train_mappo_policy \
  ...
  --policy-anchor-weight 2.0 --policy-anchor-weight-end 0.3
