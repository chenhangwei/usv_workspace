#!/bin/bash
# fresh125 pipeline: train wider post-conflict finish repair and crossing triage.
...
LOG="${LOG:-/tmp/fresh125_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh125_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh125_wide_finish_from_fresh101.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh125_checkpoints}"
...
{
  echo "========== fresh125 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh125.sh
  echo "========== fresh125 train complete; crossing focus =========="
  ...
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh125_wide_finish_from_fresh101_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh125_crossing_summary.tsv"
...
print(
    f"fresh125 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh125 crossing summary =========="
  cat "$summary"
  echo "========== fresh125 pipeline complete =========="
