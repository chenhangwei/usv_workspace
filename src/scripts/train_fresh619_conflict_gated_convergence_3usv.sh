...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh619_conflict_gated_convergence_3usv.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh619_checkpoints}"
...
echo "[fresh619] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh619] OUTPUT=$OUTPUT"
echo "[fresh619] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh619] fresh618 recipe + heading-convergence now hard-gated by low_conflict_gate (fixes 8/9 offline collision regression)"
