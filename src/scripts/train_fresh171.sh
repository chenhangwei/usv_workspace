#!/bin/bash
# fresh171: inside-goal hold plus less-suppressed after-clear finish.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh171_inside_goal_hold_from_fresh168.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh171_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh171_train.log}"
RUN_NAME="${RUN_NAME:-fresh171}"
...
{
  echo "========== ${RUN_NAME} Inside-Goal Hold (${TOTAL_TIMESTEPS}) =========="
...
echo "${RUN_NAME} inside-goal hold fit complete: $OUTPUT" | tee -a "$LOG_FILE"
