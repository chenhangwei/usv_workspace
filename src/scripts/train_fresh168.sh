#!/bin/bash
# fresh168: random safe-finish with near-goal hold and low-priority recovery.
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh168_random_hold_lowprio_from_fresh163_step1152.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh168_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh168_train.log}"
RUN_NAME="${RUN_NAME:-fresh168}"
...
{
  echo "========== ${RUN_NAME} Random Hold Low-Priority (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --random-deconflict-release-separation 2.75 \
  ...
  --random-safe-finish-weight 1.45 \
  --random-safe-finish-weight-end 1.75 \
  ...
  --random-safe-finish-min-speed-scale 0.60 \
  --random-safe-finish-hold-distance 0.95 \
  --random-safe-finish-hold-weight 1.45 \
  ...
  --random-safe-finish-omega-weight 0.10 \
...
echo "${RUN_NAME} random hold low-priority fit complete: $OUTPUT" | tee -a "$LOG_FILE"
