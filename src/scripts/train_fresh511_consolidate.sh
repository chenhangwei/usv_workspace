...
WARMSTART_FROM="${WARMSTART_FROM:-/mnt/data/checkpoints/usv_rl/fresh510_curriculum.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh511_consolidate.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh511_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-244}"

echo "[fresh511] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh511] OUTPUT=$OUTPUT"
echo "[fresh511] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh511] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
