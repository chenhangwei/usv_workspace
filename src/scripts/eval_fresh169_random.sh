#!/bin/bash
# Fixed-seed random-only screening for fresh169.
...
OUT="${OUT:-/tmp/fresh169_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh169_weak_cpa_hold_from_fresh168.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh169_checkpoints/fresh169_weak_cpa_hold_from_fresh168_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh169 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh169-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
