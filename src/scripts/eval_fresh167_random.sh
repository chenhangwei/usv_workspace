#!/bin/bash
# Fixed-seed random-only screening for fresh167.
...
OUT="${OUT:-/tmp/fresh167_random_eval}"
...
FINAL_MODEL="${FINAL_MODEL:-/mnt/data/checkpoints/usv_rl/fresh167_random_safe_finish_from_fresh163_step1152.pt}"
CKPT_MODEL="${CKPT_MODEL:-/mnt/data/checkpoints/usv_rl/fresh167_checkpoints/fresh167_random_safe_finish_from_fresh163_step1152_step_0001260.pt}"
...
eval_one() {
  ...
  echo "----- fresh167 random-only label=${label} domain=${domain} seed=${SEED} -----"
...
print(row)
print(
    f"fresh167-random {label}: coll={values[0]:.3f} succ={values[1]:.3f} "
