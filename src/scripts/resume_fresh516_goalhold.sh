...
if [[ -f "${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh516_goalhold.pt}" ]]; then
  RESUME_FROM="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh516_goalhold.pt}"
else
  RESUME_FROM="$(ls -1t /mnt/data/checkpoints/usv_rl/fresh516_checkpoints/*.pt 2>/dev/null | head -1 || true)"
fi
if [[ -z "${RESUME_FROM:-}" || ! -f "$RESUME_FROM" ]]; then
  echo "[resume516] ERROR: no checkpoint to resume from" >&2; exit 1
fi
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh516_goalhold.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh516_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-254}"

echo "[resume516] RESUME_FROM=$RESUME_FROM"
echo "[fresh516] OUTPUT=$OUTPUT"
echo "[fresh516] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh516] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
...
exec python3 -m usv_rl.train_mappo_policy \
  ...
  --near-goal-finish-weight 0.6 --near-goal-finish-weight-end 0.6 \
