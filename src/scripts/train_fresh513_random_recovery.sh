...
WARMSTART_FROM="${WARMSTART_FROM:-/mnt/data/checkpoints/usv_rl/fresh512_decouple.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh513_random_recovery.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh513_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-248}"

echo "[fresh513] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh513] OUTPUT=$OUTPUT"
echo "[fresh513] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh513] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
...
exec python3 -m usv_rl.train_mappo_policy \
  ...
  --curriculum-scenario three_usv_random_encounter \
  --curriculum-scenario three_usv_random_encounter \
  --scenario-spawn-position-std 0.12 \
  --scenario-spawn-heading-std 0.06 \
