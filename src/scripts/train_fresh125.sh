#!/bin/bash
# fresh125: wider post-conflict finish repair from the fresh101 crossing-safe base.
#
# fresh124 kept crossing safe, but the active finish band was too narrow: the
# last agent often remained 4-8m from goal after conflict separation.  This run
# widens only the trainer-side lagging-finish band while keeping a strict
# post-conflict phase gate, fleet-separation gate, and strong policy anchor.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh125_wide_finish_from_fresh101.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh125_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh125_train.log}"
export RUN_NAME="${RUN_NAME:-fresh125}"
export TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-1728}"
