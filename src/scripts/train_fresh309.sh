#!/bin/bash
# fresh309: continue rear-only overtaking from fresh308.
#
# Focus:
#   - Start from the background-safe fresh308 policy.
#   - Oversample single_usv_overtaking and clear-route discipline.
#   - Keep random hard seeds for evaluation, not this micro-stage rollout mix.

set -eo pipefail

cd "$(dirname "$0")/.."

BASE_INPUT="${BASE_INPUT:-/mnt/data/checkpoints/usv_rl/fresh308_bg_safe_overtake_from_fresh301.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh309_single_overtake_finish_from_fresh308.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh309_checkpoints}"
...
FRESH309_ARGS="
  ...
  --curriculum-seed 3073
  --curriculum-seed 3074
  --curriculum-seed 3081
  --curriculum-seed 3082
  ...
  --random-clear-ahead-weight 6.20
  ...
  --random-deconflict-omega-weight 1.10
  --random-deconflict-standon-weight 0.18
  --random-deconflict-yield-weight 4.40
  --random-deconflict-pretrain-epochs 1
  --random-deconflict-pretrain-learning-rate 2.0e-7
  --random-deconflict-pretrain-max-grad-norm 0.025
  --random-role-balance-weight 3.80
  --random-role-balance-weight-end 4.20
  --random-role-balance-pretrain-epochs 1
  --random-role-balance-pretrain-learning-rate 2.0e-7
  ...
  --random-role-balance-yield-weight 2.00
  --random-role-balance-omega-weight 0.34
  --random-cte-recovery-weight 0.20
  --random-cte-recovery-weight-end 0.26
  ...
  --random-safe-finish-weight 0.00
  --random-safe-finish-weight-end 0.00
  ...
  --random-offroute-finish-weight 0.00
  --random-offroute-finish-weight-end 0.00
  --policy-anchor-weight 1200.0
  --policy-anchor-weight-end 1800.0
