#!/bin/bash
# fresh132 pipeline: train neighbor-localized separation polish and run crossing triage.
...
LOG="${LOG:-/tmp/fresh132_pipeline.log}"
OUT="${OUT:-/mnt/data/checkpoints/usv_rl/fresh132_crossing_focus_eval}"
EPISODES="${EPISODES:-1}"
STEPS="${STEPS:-400}"
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh132_neighbor_sep_floor_from_fresh124_step0576.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh132_checkpoints}"
...
FRESH129="${FRESH129:-/mnt/data/checkpoints/usv_rl/fresh129_finish_regain_from_fresh128.pt}"
export PYTHONUNBUFFERED=1

mkdir -p "$OUT"

{
  echo "========== fresh132 pipeline =========="
  echo "Started: $(date -Iseconds)"
  bash src/scripts/train_fresh132.sh
  echo "========== fresh132 train complete; crossing focus =========="

  cd src
  declare -a candidates=()
  [[ -f "$FRESH124" ]] && candidates+=("$FRESH124")
  [[ -f "$FRESH128" ]] && candidates+=("$FRESH128")
  [[ -f "$FRESH129" ]] && candidates+=("$FRESH129")
  [[ -f "$MODEL" ]] && candidates+=("$MODEL")
  if [[ -d "$CKPT_DIR" ]]; then
    while IFS= read -r ckpt; do
      candidates+=("$ckpt")
    done < <(ls -1t "$CKPT_DIR"/fresh132_neighbor_sep_floor_from_fresh124_step0576_step_*.pt 2>/dev/null || true)
  fi
  ...
  summary="$OUT/fresh132_crossing_summary.tsv"
...
print(
    f"fresh132 crossing {label}: coll={row['collision']:.3f} "
...
PY
  ...
  echo "========== fresh132 crossing summary =========="
  cat "$summary"
  echo "========== fresh132 pipeline complete =========="
