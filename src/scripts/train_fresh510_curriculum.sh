...
WARMSTART_FROM="${WARMSTART_FROM:-/mnt/data/checkpoints/usv_rl/fresh509g_loosen.pt}"
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh510_curriculum.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh510_checkpoints}"
...
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-242}"

echo "[fresh510] WARMSTART_FROM=$WARMSTART_FROM"
echo "[fresh510] OUTPUT=$OUTPUT"
echo "[fresh510] TOTAL_TIMESTEPS=$TOTAL_TIMESTEPS  ROLLOUT_STEPS=$ROLLOUT_STEPS"
echo "[fresh510] BASE_ROS_DOMAIN_ID=$BASE_ROS_DOMAIN_ID"
...
exec python3 -m usv_rl.train_mappo_policy \
  ...
  --scenario-spawn-heading-std 0.12 \
