#!/bin/bash
# fresh131: activation-unblocked separation-floor polish from fresh124 step0576.
#
# fresh130 proved that the narrow team-safety brake gate never activated: all
# checkpoints kept identical actor/critic tensors to fresh124.  This run keeps
# PPO disabled and widens only the safety-brake active mask so the auxiliary can
# actually sample close-team states without adding finish pressure.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh131_active_sep_floor_from_fresh124_step0576.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh131_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh131_train.log}"
export RUN_NAME="${RUN_NAME:-fresh131}"
...
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:-} \
  ...
  --team-safety-brake-weight 0.16 \
  --team-safety-brake-weight-end 0.06 \
  ...
  --team-safety-brake-release-separation 2.75 \
  --team-safety-brake-target-speed 0.16 \
  --team-safety-brake-omega-weight 0.015 \
  --team-safety-brake-target-omega 0.025 \
  --team-safety-brake-turn-mode away \
  --team-safety-brake-power 2.40 \
