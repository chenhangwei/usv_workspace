#!/bin/bash
# Three-episode confirm for a selected fresh155 checkpoint.
...
MODEL="${MODEL:-/mnt/data/checkpoints/usv_rl/fresh155_reset_curriculum_cpa_finish_from_fresh153.pt}"
LABEL="${LABEL:-$(basename "$MODEL" .pt)}"
SUMMARY="${SUMMARY:-$OUT/fresh155_confirm_summary.tsv}"
EPISODES="${EPISODES:-3}"
STEPS="${STEPS:-420}"
...
print(
    f"fresh155-confirm {label} {scenario_name}: episodes={row['episodes']} "
...
printf '========== fresh155 confirm summary =========='"\n"
cat "$SUMMARY"