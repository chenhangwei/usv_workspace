#!/bin/bash
# fresh121: early crossing team safety brake from fresh109.
#
# fresh120's post-goal brake never activated, so the safety target must move
# earlier than team completion. This keeps the pure single MAPPO checkpoint and
# trains low forward speed for unfinished crossing agents whenever fleet
# separation is compressed, without adding evaluator/deployment rules.
...
export BASE_INPUT
export OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh121_early_team_brake.pt}"
export CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh121_checkpoints}"
export LOG_FILE="${LOG_FILE:-/tmp/fresh121_train.log}"
export RUN_NAME="${RUN_NAME:-fresh121}"
