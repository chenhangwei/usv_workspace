#!/bin/bash
# fresh146: clear-ahead gated route discipline from fresh142.
#
# Direction:
#   - do not continue fresh145's global strong CTE/omega punishment
#   - keep close-margin/finish behavior from fresh142/fresh144
#   - apply extra CTE/heading/omega discipline only when the forward cone is clear
...
OUTPUT="${OUTPUT:-/mnt/data/checkpoints/usv_rl/fresh146_clear_ahead_route_gate_from_fresh142.pt}"
CKPT_DIR="${CKPT_DIR:-/mnt/data/checkpoints/usv_rl/fresh146_checkpoints}"
LOG_FILE="${LOG_FILE:-/tmp/fresh146_train.log}"
RUN_NAME="${RUN_NAME:-fresh146}"
TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS:-864}"
BASE_ROS_DOMAIN_ID="${BASE_ROS_DOMAIN_ID:-200}"
...
{
  echo "========== ${RUN_NAME} Clear-Ahead Route Gate (${TOTAL_TIMESTEPS}) =========="
...
/bin/python3 -m usv_rl.train_mappo_policy \
  ...
  --clear-ahead-bearing-deg 32.0 \
  --clear-ahead-cte-weight 2.8 \
  --clear-ahead-heading-weight 0.95 \
  --clear-ahead-omega-weight 7.0 \
...
echo "${RUN_NAME} clear-ahead route gate complete: $OUTPUT" | tee -a "$LOG_FILE"
