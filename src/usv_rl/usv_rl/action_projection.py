...
def _resolve_turn_authority(
    *,
    ...
    base_authority = max(raw_heading_ratio, _clamp(float(conflict_turn_relief), 0.0, 1.0) * _clamp(float(conflict_level), 0.0, 1.0))
...
def project_pure_action(
    ...
    *,
    ...
    if heading_error is None or current_angular_z is None or control_dt is None:
        ...
        drive_ratio = max(heading_ratio, _clamp(float(conflict_level), 0.0, 1.0))
