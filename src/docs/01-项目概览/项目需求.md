...
### 3.4 任务区域与随机运行围栏
- **Area Center (任务原点)**：
  - 定义任务坐标系原点在全局 Map 坐标系中的位置（X, Y, Z）。
  - 任务 XML 中的坐标相对于该原点，GS 在下发目标前会将其转换为全局坐标。
- **Fence (随机运行围栏)**：
  - 在“随机运行”模式下，GS 会在围栏范围内为每艘在线且处于 OFFBOARD 模式的 USV 生成随机目标点。
  - 支持两种形状：
    - **圆柱形 (Cylinder)**：由半径和高度定义。
    - **长方体 (Box)**：由长度 (X)、宽度 (Y) 和高度 (Z) 定义。
  - 围栏中心默认与 Area Center 重合。
  - 随机点生成时应保留一定的边缘余量（如 10%），避免 USV 频繁触碰边界。
